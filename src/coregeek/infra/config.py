...
#: ✅任务书 L205：「武器工事全局同时最多建造 3 座，新建的武器等级为 level1」。
#: ✅任务书 §4.5.1：三种武器的**初始数量都是 0**；§4.5.3：**开局金币 75**。
#: 即 75 / 25 = 恰好 3 座 —— **开局全款买满武器是设计好的**，
#: 第 1 天白天的第一优先动作就是建这 3 座（见 domain/economy.next_weapon_build）。
#:
#: ⚠️ 这里曾经写反过：早期把 `docs/request.txt`（那是 `roundNo=85` 的**中局**快照，
#: 已有 3 座武器 + 20 金）当成了"开局状态"，于是判定"开局已送 3 座、造武器是死支出线"，
#: 把预算全推给了升级券。后果是**第 1 天一座武器都不造**，入夜三人无武器可操控。
#: 25×3 = 75 = 初始金币，正是那条推理错误的直接反证。
MAX_WEAPONS = 3

#: 武器建造顺序：按 `Box.weapon_sites()`（由正面往后）**依次填**。
#: 射程越短的越靠前 —— 加特林 L1 射程只有 3，放背面等于废掉；火箭 L1 射程 10，
#: 放哪都覆盖全场。用户选定"加特林 / 电磁 / 火箭 各一座"（与样例配置一致）。
WEAPON_LOADOUT: tuple[str, ...] = ("gatling", "railgun", "rocket")
WALL_MATERIAL = "stone"
ORE_KINDS = ("stone", "iron", "copper")
STONE_BUFFER = 3  # stone_target = 缺口面数 + STONE_BUFFER
#: 筑墙手**攒够多少块石头再回去建墙**。
#: 不设这个阈值的话，工人会"挖一块→走回去建一块→再走回来挖"，每个缺口两趟往返；
#: 一个盒子 19 面墙就是 38 趟。攒到 6 块再动身，往返次数降到 ~4 趟。
#: 取 6 而不是 19（一次攒满）是为了让正面墙**早点立起来** —— 夜战在第 71 回合。
STONE_STOCK = 6
