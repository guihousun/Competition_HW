...
一条指令就是一个对象，创建即校验：`roleType` 传错抛 `PermissionError`，非法动作
根本造不出来。"指令非法"计异常（5 次出局），而"角色没权限"算不算异常文档没写清，
只能让它发不出去。权限写窄了本地测不出来——报文照样合法，只有判题器会说"不"。
子类落地补四样：`code`（动作码）、`roles`（可用角色，任务书 §4.4 表最右列）、
...
def describe(cmds: dict[str, Any], *, clip: Callable[[str], str]) -> str:
    ...
    字段是通用摊开的：`action` 之外的每个字段一律 `名=值` 印出来，`targetPos` 折成
    `(x,y)`。不手写"哪个动作有哪几个字段"的清单——那是会跟报文漂移的第二份真相，
    漏打一个字段不像崩溃那么显眼；而且 `acceptTask` / `submitAnswer` 压根没有
    `targetPos`，硬读会 `IndexError` ⇒ 整回合退化成空指令。`clip` 由调用方注入
    （唯一使用者是 `app._log`）：截断规则属于日志层，这条边是注入而不是 import——
    `describe` 的契约是"不认识日志"，字符串值一律过它。
    """
    parts = []
...
class Build(BaseAction):
    ...
    `name` 取 `roleType` 同名（`wall` / `gatling` / `railgun` / `rocket`）。目标必须是
    空的可建造格：落在已有武器上，原武器会被覆盖成 level1（25 金币白花还降级）。
    """
...
class Remove(BaseAction):
    ...
    拆完那格变可穿越空地，但不回收建造时花掉的石头——拆墙不是免费的。
    实证报文（`docs/response.txt`）：`{"action":"remove","targetPos":[{"x":29,"y":7}]}`
    ——没有 `name` 字段（`build` 才有），多打一个字段就是赌"指令非法"。
    昼夜限制任务书没写，按"仅白天"保守执行：夜里不发最多少拆一次，反之若实际禁夜
    就是指令非法。打空处 / 目标不是己方围墙 ⇒ 执行失败，不计异常。
    """
...
class Collect(BaseAction):
    ...
    能力列没有昼夜限制（`build` 写"仅白天"、`attack` 写"仅黑夜"，它那格是空的）
    ⇒ 昼夜门由 `planner` 把关，不进闸门（闸门只管"谁"）。`targetPos` 是矿石的坐标，
    不是自己的站位；须站在矿的切比雪夫 ≤1 内（矿格挡路，"站在矿上"不可能）。
    一次采 1 块，一座矿采满 10 次就消失，payload 没有"剩余次数"字段。
    形状与 `move` 同形，没有实证报文。
    """
...
class Attack(BaseAction):
    ...
    `controllerId` 里——`planner` 那边要用 `_emit(..., key=str(weapon.id))`。
    实证报文：`{"action":"attack","controllerId":"10010","targetPos":[{"x":29,"y":7}]}`
    两条站位硬规则（不满足是"指令执行失败"，不计异常，但白打一发）：操控者须站在
    武器周围一格内，且一人同时只能操控一座；目标格须在射程内（打空处 = 执行失败）。
...
class Sell(BaseAction):
    ...
    "本地全绿、判题器说不"：报文完全合法，只有判题器知道开拓者也能卖。
    能力列没有昼夜限制 ⇒ 昼夜门由 `planner` 把关，不进闸门。`name` 是矿种（英文，
    与 `neutralType` / `vendorShopList.name` 同一套词），`num` 是批量卖的件数
    （Int，不填默认 1）。形状从接口文档推的，没有实证报文。
    """
...
class Buy(BaseAction):
    ...
    `num` 是 Int（不填默认 1，支持批量）。能力列没有昼夜限制 ⇒ 昼夜门由 `planner`
    把关（这条线只在白天跑）。形状从接口文档推，没有实证报文（与 `sell` 同一类风险）。
    """
...
class Use(BaseAction):
    """使用背包里的一件商品。可用角色是"全部"。目前只用于升级券：
    须站在目标建筑周围一格内并指定 `targetPos`（任务书 L292）⇒ `target` 必填。
    已 level3 再用不生效、券不消耗；非法使用也不消耗（L293-294）⇒ 发错顶多
    白跑一趟，不碰红线。形状从接口文档推，没有实证报文。
    """
...
class AcceptTask(BaseAction):
    ...
    报文里没有 `targetPos`——领哪个任务点由站位决定（须在己方任务点周围一格内）。
    实证报文：`{"10011":{"action":"acceptTask"}}`。在敌方任务点执行无效，
...
class SubmitAnswer(BaseAction):
    ...
    可以反复提交：判题器按"提交过的通过率最高的答案"算分，重复提交是预期用法。
