...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    1000 → **100000** ⇒ **日志基本不截**（第 28 步起 prompt 是**单行** messages JSON ——
    换行都被转义，带提问的回合行数大幅下降）：

    | 局面 | 行 | 字节 |
    |---|---|---|
    | 干净回合（没回执、没任务） | 7 | 593 |
    | 有回执（判题器报错 + 回执名单） | 9 | 741 |
    | 提问那一轮（题目 400 字） | 9 | **6497** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈604147** |

    ⚠️ **第 36 步（SOP 工具只留 `sop` 一个参数 + 答案只走 `<answer>` + SOP 禁含 `<answer>` 对）
    把提问那轮从 6164 抬到 6497、顶格从 603814 抬到 604147** —— 两行同为 **+333**，
    全部来自 system 模板（换掉那条"答案塞进 `answer` 参数"的示例、改成"工具块 + 块外
    `<answer>`"两块，再加一句"`sop` 里不许出现这对标签"）。**只有带 prompt 的那两行会动**：
    干净回合 593 与有回执 741 在改动前后**逐字节相同**（它们没有 prompt 行 ⇒ 模板改不到）。
    （第 35 步记的是同一条规律：5733 → 6020、603375 → 603662，同为 +287。）
    ⚠️ **表里的绝对值是第 36 步用"同一脚本跑新旧两版"重新量的** —— 与第 26/35 步记的
    6020 / ≈603662 差 +477/+485，成因是构造（`errors`/`action_results`/`llmResp` 怎么铺）与
    `request.txt` 的内容漂移，**跨步骤比绝对值没有意义，同一次测量的增量才是信号**
    （"有回执"那行从记过的 795 变成 741 而同版本同值，就是这条的实例）。

    **量法**（第 32 步起写在文档里，免得重测时靠回忆凑构造）：拿 `docs/request.txt` 改
    `roundNo=1`、把 `phaseTask`/`llmResp`/`lastCmdResult` 换成题目那种字符，`errors` 与
    `lastRoundRoleActionResults` 两格决定"有回执"那一行。
    ⚠️ **第 36 步补的两条**（否则重测出来的绝对值又对不上）：① 量的是**真 stdout 的形状** ——
    给 root logger 挂一个 `Formatter("%(asctime)s | %(message)s")` 的 handler，取
    `len(捕获到的文本.encode("utf-8"))`（**别只加 `getMessage()` 的长度**，那会漏掉时间戳前缀）；
    ② **每格先 `AGENT.reset()`**（prompt 里带着 SOP 与会话，前一格的沉淀会灌进下一格）。
    想要一个**可跨步骤比较**的数，就把同一脚本在 `git worktree add ../x HEAD --detach`
    出来的旧版上再跑一遍 —— 绝对值受构造与 `request.txt` 漂移影响，**增量不受**。⚠️ **顶格那行的 `llmResp` 必须是
    「工具形状」**（`<tool>…</tool>`）—— 若写成答案形状，同一个回复会被 `_answer_task` 抄进
    `submitAnswer` 的 `taskAnswer`，动作行凭空多出 120KB（量出来是 ≈723000，不是顶格的真实值）。
    前两格的 ±几字节随 `request.txt` 当天的内容漂移，只有量级有意义。
