...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    ⚠️ **第 46 步（ROLE 段环境知识沉淀提示 + SOP 工具描述放宽）两格同为 +614**
    （同一次测量的新旧对照：提问 8300 → 8914、顶格 605948 → 606562；干净/有回执逐字节
    不动）。⚠️ **第 45 步（`python_exec` 工具进 system）两格同为 +604**（同一次测量的新旧对照：
    提问 7696 → 8300、顶格 605344 → 605948；干净/有回执逐字节不动 —— 不含 prompt 的
    局面看不见 system）。⚠️ **第 41 步（压缩独立成轮）把提问那轮从 8382 降到 7701、顶格从
    605957 降到 605276** —— 两行同为 **−681**（任务 system 卸掉第 39 步搭车的 `<summary>`
    教学；压缩指令只进命令轮的压缩请求，不进任务 system）。⚠️ **命令轮现在多出一条 prompt
    行（压缩请求）**：它随原始历史线性变大、`LOG_PROMPT_MAX` 兜底截断留痕 ——
    上表四格没有这一行（"提问那轮"量的是任务首问）。历史：第 39 步 +881、
    第 38 步 +546、第 37 步 +467。**只有带 prompt 的那两行会动**。**跨步骤比绝对值没有
    意义，同一次测量的增量才是信号**（行数列是第 26 步量法的物理行，第 45 步起只动字节列）。
