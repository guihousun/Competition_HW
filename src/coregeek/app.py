...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    | 提问那一轮（题目 400 字） | 9 | **7701** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈605276** |

    ⚠️ **第 41 步（压缩独立成轮）把提问那轮从 8382 降到 7701、顶格从 605957 降到
    605276** —— 两行同为 **−681**（任务 system 卸掉第 39 步搭车的 `<summary>` 教学；
    压缩指令只进命令轮的压缩请求，不进任务 system）。⚠️ **命令轮现在多出一条 prompt
    行（压缩请求）**：它随原始历史线性变大、`LOG_PROMPT_MAX` 兜底截断留痕 ——
    上表四格没有这一行（"提问那轮"量的是任务首问）。历史：第 39 步 +881、
    第 38 步 +546、第 37 步 +467。**只有带 prompt 的那两行会动**（干净回合 593 与
    有回执 741 历步逐字节相同）。**跨步骤比绝对值没有意义，同一次测量的增量才是信号。**
