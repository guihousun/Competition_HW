...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    | 提问那一轮（题目 400 字） | 9 | **7501** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈605076** |

    ⚠️ **第 38 步（用户启用 `EXAMPLE_PROMPT` 示例段）把提问那轮从 6955 抬到 7501、
    顶格从 604530 抬到 605076** —— 两行同为 **+546**，全部来自 system 段模板长大一次。
    第 37 步那次（6488 → 6955 / 604063 → 604530，同为 +467）、第 36 步（+333）、
    第 35 步（+287）抬的是同一原因。**只有带 prompt 的那两行会动**：干净回合 593 与
    有回执 741 在历步改动前后**逐字节相同**（它们没有 prompt 行 ⇒ 模板改不到）。
    ⚠️ **跨步骤比绝对值没有意义，同一次测量的增量才是信号**（构造与 `request.txt`
    会漂移；"有回执"那行从记过的 795 变成 741 而同版本同值，就是这条的实例）。
