...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    |---|---|---|
    | 干净回合（没回执、没任务） | 7 | 512 |
    | 有回执（判题器报错 + 回执名单） | 9 | 606 |
    | 提问那一轮（题目 400 字） | 9 | **6020** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 11 | **≈603662** |

    ⚠️ **第 35 步（`answer` 进工具参数 + prompt 加「工作流」）把提问那轮从 5733 抬到 6020、
    顶格从 603375 抬到 603662**（两行同为 **+287** = system 段长大的那部分）。干净回合与
    有回执那两行在改动前后**逐字节相同**（它们没有 prompt 行 ⇒ 模板改不到）；与更早文档
    记的 514/611 差 2~5 字节是 `request.txt` 当天的内容漂移。

    **量法**（第 32 步起写在文档里，免得重测时靠回忆凑构造）：拿 `docs/request.txt` 改
    `roundNo=1`、把 `phaseTask`/`llmResp`/`lastCmdResult` 换成题目那种字符，`errors` 与
    `lastRoundRoleActionResults` 两格决定"有回执"那一行。⚠️ **顶格那行的 `llmResp` 必须是
