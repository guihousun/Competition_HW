...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **写在 `try` 里面**：日志代码再不起眼也是代码。逃到 `do_POST` 去的话，
    ...
    ⚠️ **stdout 是会被写满的**：每回合 40 行 / **2262 字节**（第 12 步实测，改前 33 行 / 1374 字节）
    × 1300 回合 ≈ **2.9 MB**，而 Windows 管道缓冲只有 64KB —— 判题器若**不读** stdout，
    约 **29 回合**后这里就阻塞到响应超时。**未实测**（本地没法验证判题器读不读），
    真出事只能改这一处。**数字与 `CLAUDE.md` 硬约束 5 必须一致**（留着旧数字会让下一个人按错的量级估风险）。
    """
    LOGGER.info(
