...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    | 提问那一轮（题目 400 字） | 9 | **6955** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈604530** |

    ⚠️ **第 37 步（prompt.py 接管 system 模板 + 协议换嵌套形状 + SOP 流程表）把提问那轮
    从 6488 抬到 6955、顶格从 604063 抬到 604530** —— 两行同为 **+467**，全部来自 system
    段模板长大一次（六段式更长：嵌套示例 + `<name>` 行 + 流程表说明）。**只有带 prompt 的
    那两行会动**：干净回合 593 与有回执 741 在改动前后**逐字节相同**（它们没有 prompt 行 ⇒
    模板改不到）。（第 36 步记的是同一条规律：6164 → 6497 / 603814 → 604147，同为 +333；
    第 35 步：5733 → 6020 / 603375 → 603662，同为 +287。）
    ⚠️ **表里的绝对值是第 37 步用"同一脚本跑新旧两版"重新量的** —— 与第 36 步记的
    6497 / ≈604147 差 −9/−84，成因是构造（`errors`/`action_results`/`llmResp` 怎么铺）与
