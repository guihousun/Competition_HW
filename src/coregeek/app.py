...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **第一条物理行**上，拆开之后地图那几十行就没有时间戳了。
    ...
    | 任务在身、还没答过（**提问那一轮**） | 69 | 6170 | 约 10 回合 |
    | 任务在身 + 沙盒结果顶格（回灌那一轮） | 70 | 8572 | 约 7 回合 |
    | **最坏：上面全部 + 一回合 SOP 调用顶格** | 71 | **8861** | 约 **7 回合** |
    ...
    **截断必须留痕**：`_clip(text, limit)` 超长时打 `…（共 N 字）`，两个上限的单位都是**字**
    ...
    # ── 任务线 ──────────────────────────────────────────────────────
    if prompt:
        # `prompt` **打全文**：它是拼出来的（模板 + 沉淀的 SOP + 沙盒结果 + 纠错 + 题目），
        # 工具清单长什么样、SOP 那个槽填进去没有，**实盘上只有这一行能回答**。
