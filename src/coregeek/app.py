...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    | 提问那一轮（题目 400 字） | 9 | **8300** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈605948** |

    ⚠️ **第 45 步（`python_exec` 工具进 system）两格同为 +604**（同一次测量的新旧对照：
    ...
    意义，同一次测量的增量才是信号**（行数列是第 26 步量法的物理行，第 45 步只动字节列）。
