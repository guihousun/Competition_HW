...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条
    ...
    ⚠️ **stdout 是会被写满的**（第 18 步重测，样例 `request.txt`，每一格都从"没沉淀过 SOP"
    起算 —— SOP 段也在 prompt 里，会连带改变"提问：有（N 字）"那个数字）：

    | 局面 | 行 | 字节 | 写满 64KB |
    |---|---|---|---|
    | 干净回合（没回执、没任务） | 41 | 2329 | 约 28 回合 |
    | 有回执（判题器报错 + 两条未通过） | 43 | 2398 | 约 27 回合 |
    | 任务在身（`phaseTask`/`llmResp` 都顶到 `LOG_TEXT_MAX`） | 42 | 4770 | 约 14 回合 |
    | 任务在身 + 沙盒结果顶格 | 43 | 6002 | 约 11 回合 |
    | **最坏：上面全部 + SOP 刚更新** | 44 | **6180** | 约 **10 回合** |

    任务线那两条是**大头**，而且**任务期间每回合都打**（同一个任务原文重复打几十遍）。
    ...
    1300 回合 ≈ 3 MB（无任务）～ 8 MB（全程有任务），而 Windows 管道缓冲只有 64KB
    —— 判题器若**不读** stdout，最坏约 **10 回合**后这里就阻塞到响应超时。**未实测**
    （本地没法验证判题器读不读），真出事只能改这一处（把 `LOG_TEXT_MAX` 调小最直接）。
    **数字与 `CLAUDE.md` 硬约束 5、以及与用例 `test_the_worst_round_stays_under_the_budget`
    三处必须一致** —— 留着旧数字会让下一个人按错的量级估风险。

    ⚠️ **旧数字（2294 / 2459 / 4832 / 6153）是错的，第 18 步一并改掉**：它们与用例实测的
    6002 从来就没对上过（docstring 记的是一次早期测量的旧局面）。现值是**用同一份
    fixture 重新跑出来的**，与用例里那 6900 的上限同源。
    """
    LOGGER.info(
    ...
    if turn.phase_task or turn.llm_resp:
        ...
        #
        # `prompt` **不打印原文**：它就是模板套上任务原文（外加沙盒结果 / 纠错 /
        # 「沉淀的 SOP」那几段），全文等于把任务抄第二遍（白花一倍字节），
        # 有意义的只是"这一回合到底问没问"。
        LOGGER.info(
            "任务：%s ｜ 提交：%s ｜ 提问：%s",
