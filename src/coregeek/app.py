...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    ...
    | 提问那一轮（题目 400 字） | 9 | **8382** |
    | **顶格：题目/回复/沙盒各 40000 字（`LOG_TEXT_MAX`）** | 10 | **≈605957** |

    ⚠️ **第 39 步（上下文压缩：`<summary>` 搭车块 + 渲染窗口）把提问那轮从 7501 抬到
    8382、顶格从 605076 抬到 605957** —— 两行同为 **+881**（输出约定第 4 条的 teaching
    文本）。⚠️ **这张表量的是"压缩前的最坏单轮"**：压缩的收益在**多轮长任务**上
    （prompt 从 O(全部历史) 变成 O(窗口)，64KB 回执堆不出来），单轮的固定开销反而
    多了这一段 —— 这是**已知并接受的代价**。历史：第 38 步 +546（示例段）、第 37 步
    +467、第 36 步 +333、第 35 步 +287。**只有带 prompt 的那两行会动**：干净回合 593
    与有回执 741 在历步改动前后**逐字节相同**（它们没有 prompt 行 ⇒ 模板改不到）。
    **跨步骤比绝对值没有意义，同一次测量的增量才是信号。**
