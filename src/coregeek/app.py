...
**每回合记一份复盘日志**（`_log`）：局面 → 本回合的动作 → 判题器的回执 → 任务线。
...
#: 一个字段，而是「模板（591 字，含工具清单）+ 沉淀的 SOP + 沙盒结果 + 纠错 + 题目」拼出来的，
...
def _log(turn: Turn, cmds: dict[str, dict[str, Any]], prompt: str) -> None:
    ...
    **记录数不固定，但每条的触发条件都是"有事才吭声"**：局面与动作每回合各一条；
    判题器报错 / 有回执 / 任务线 / 沙盒结果四条各自只在有内容时出现（干净的白天回合 2 条）。
    ...
    **截断必须留痕**：`_clip(text, limit)` 超长时打 `…（共 N 字）`，两个上限的单位都是**字**
    ...
    if turn.phase_task or turn.llm_resp:
        ...
        # "判题器最后答了什么"的机会。`prompt` 打全文 —— 它是拼出来的，**别处看不见**
        # （模板里的工具清单长什么样、SOP 那个槽填进去没有，实盘上只有这一行能看到）。
        LOGGER.info(
            "任务：%s ｜ 提交：%s ｜ 提问：%s",
    ...
    if turn.cmd_result:
        ...
        # 已经印在上面那行的"提交："里了；而且这样"沙盒行数 = 实际跑过的命令数"。
