"""决策入口：`Turn` → `roleCommandMap`。只做胶水 —— 两段式组装 + 那条按处境分派的判据链。
...
def _intents(turn: Turn) -> tuple[_Queue, set[Pos]]:
    ...
    黑板装在 `_Ctx` 里逐角色顺序累计、不跨回合。工人的判据链在 `day.DAY_CHAIN`。"""
    q = _Queue(turn)
    cmds = q.cmds
    ...
    for role in turn.roles:
        ...
        if turn.is_day and day.rescue(role, turn, q, box):
            continue
        ...
        # 白天收工：环砌完了、离夜里第一波只剩回程步数才回家（判据在 `day.BackToPost`）
