...
def task_channel(turn: Turn) -> tuple[str, str]:
    ...
    4. **判题器说答案错了 ⇒ 带着"上次答错了"重问**（`errors` 里有 `code == 2` 且回复不是工具调用）。
       ...
       第 47 步起**拿不到答案原文也照样纠错**：判题器的反馈（`description` 原话）装进
       提示词，绝不落「请继续。」—— 那等于没告诉它答案错了。
    5. **回复是最终答案 ⇒ 只交答案**：不发模型请求、**也不压缩**（第 47 步：压缩与
       `<answer>` 互斥），开拓者那边每回合在 `submitAnswer`。
    ...
    **回合最末尾另有一道压缩闸门（第 43 步；第 47 步收窄到只剩 ③）**：判据链算完
    `prompt` 还是空（只有 ③ 命令轮会这样 —— ⑤ 答案轮提前返回了，压缩与 `<answer>`
    互斥）⇒ 填上**压缩请求**。②/④/⑥ 的模型请求永远优先，压缩是这条链上优先级最低的
    租客；判据 ① 的两条早返回在闸门**之前** —— 没任务/没开拓者绝不压缩（任务线之外
    每游戏日只有 3 次额度）。压缩请求的回复（裸 `<summary>`）在链首就被路由进摘要、
    不进会话表，下一轮判据按"没回复"落 ⑥ 把任务对话推回去 —— 于是命令轮与压缩轮
    **交替**，互不阻塞。没开过会话 ⇒ `compression_request` 给 `""`，维持旧形状（降级安全）。
    ...
    **本函数还负责打"本轮任务 / 上一轮模型回复 / CMD 执行结果"三样**（它们只存在于本回合的
    ...
    if summary is not None:
        # **压缩回复**（压缩请求的产物 —— 只跟在 ③ 命令轮后面；第 47 步起答案轮不压
        # 缩，压缩与 `<answer>` 互斥）：进摘要、
    ...
    #: ⚠️ **骂的那一份必须与交的那一份同源**：交上去的是解包后的答案，骂的不能是
    #: 带标签的原文。判题器没给描述（`why` 空）⇒ 只骂答案本身，不为"少了一句话"
    #: 把整段纠错吞掉（那是判题器侧的信息缺失，不是我们该丢的信心）。
    #: **第 47 步（用户口径）**：`code 2` 而这轮回复里拿不到答案原文（`llmResp`
    #: 文档没写 ⇒ 必须按可能不粘设计）⇒ **反馈照样装进提示词**，绝不落「请继续。」
    #: —— 那等于没告诉它答案错了。反馈-only 的重问里没有答案文本，同源那条无从踩雷；
    #: 会话里最后一条 assistant 正是它上一次说过的话，反馈紧跟着落，语义完整。
    retry = ""
    ...
    elif answer:  # ⑤ 拿到了答案 ⇒ 只交答案：不发模型请求、也不压缩（第 47 步）
        # 压缩与 `<answer>` 互斥：压缩回复会占住下一轮的 llmResp 槽，答案被判错时
        # 纠错分支就拿不到答案原文；任务的最后一回合更是白压。提前返回 =
        # 压缩闸门从此只填 ③ 命令轮的空槽。
        return "", ""
