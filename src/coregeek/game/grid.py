...
def wall_cells(base: Pos, width: int) -> tuple[Pos, ...]:
    ...
    **背面中间 2 格（基地纵深中心那一行及其下一行）永远是空地**（`door_cells` 给的就是它）：
    它是工人进出、采石、
    跑商店的唯一通道。环一闭合工人就进出不得，而 `remove`（拆墙）没实现 ⇒ 关进去就是
    整场出不来（第 33 步把门从"整列 6 格"收成这 2 格：入口从 6 路并行变 2 路串行，
    ...
    **封口格 = 正面列正中 `(front_x, by)`，排在最后一个** —— 这是"白天开着方便通行、
    ...
    ⚠️ **砌上就拆不掉**（没有 `remove`）⇒ "白天开口"只在**第 1 天**生效；
    之后正面永久封死，工人一律走背面那 2 格门。这是有意的：通行口的主要价值在前 1 天
    （墙还在砌、人频繁进出盒子），之后人基本在盒外干活、天黑才回。
