...
def weapon_sites(base: Pos, width: int) -> tuple[Pos, ...]:
    """三座武器的**落点，按建造顺序**：前排上方相邻两格（2 火箭）+ 前排下方一格（加特林）。

    两火箭相邻 ⇒ 一个角色站在它们**内侧那一格**（武器环里的空格）就能同时贴着两座，
    利用火箭 3 回合冷却交替开火 —— **2 个角色即可操作 3 座武器**，释放开拓者夜间行动。
    放弃后列与电磁炮：电磁炮单目标、对满血机器人不穿透，群体价值最低；双火箭的溅射
    在机器人挤在墙根时收益极高，且射程远（L1 10、L3 全图）。

    正/背面由 `_front_back` 给（左半基地 `front_x = bx+2`、右半 `front_x = bx-1`）：
    - **前排上两格 `(front_x, by)` 与 `(front_x, by+1)`**：相邻，火箭落点；
    - **前排下格 `(front_x, by-2)`**：加特林落点。

    左半基地 `(10, 24)` ⇒ `((12, 24), (12, 25), (12, 22))`。三个都在 `weapon_cells` 里。
    两火箭的操作位是内侧那一格 `(bx+1, by+1)` = `(11, 25)`（同时贴着两座火箭）。
    """
    d, far, near = _front_back(base, width)
    front_x = far + d
    return (
