...
class Turn(NamedTuple):
    ...
    llm_resp: str = ""
    #: 上回合 `executeCmd` 的执行结果（接口文档 L33）—— **沙盒交回来的作业**。
    #: 文档明说"**未发命令时为空字符串 `""`**" ⇒ 非空就等于"我上回合发过命令、这是回执"，
    #: 任务线靠它决定"该把结果回灌给 LLM 了"。
    #: 格式约定（`[exitCode:N]` / `[TIMEOUT]` / `[JUDGER_ERROR]` 打头，超 64KB 尾加
    #: `[TRUNCATED]`）**不解析** —— 非空即原文回灌，那行状态让 LLM 自己读。
    cmd_result: str = ""
