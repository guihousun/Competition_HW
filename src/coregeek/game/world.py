...
#: 三种武器工事的 `roleType`。与 `planner.WEAPONS_BY_SITE` 不是一回事：
...
class Weapon(NamedTuple):
    ...
    `level` 缺省 1（仅建筑持有、初始 1）—— 升级线靠它认"还升得动"。没有 `health`：
    只在解析时用它筛掉已毁的炮，存下来没人读。
    """
    ...
    level: int = 1
    """当前等级（1..3）。默认 1：缺失按"初始等级 1"算，别当 -1
...
class Wall(NamedTuple):
    ...
    与武器同住 `teamOur.roles`（`roleType == "wall"`，id 40000 系）。health 是修墙的
    判据（半血 ⇒ 修复包/重建）；level 决定满血基准（升级后回满血）。已毁（health == 0）
    的墙在 `model._walls` 就丢掉 —— 那是一格缺口，归 `_ring` 管。
    """
...
class Robot(NamedTuple):
    ...
    `id` / `roleType` / `abnormalState` 都不存 —— 目标位置用的是坐标。
    `target_team`：该机器人打哪一队（"challenger" / "defender"），用于过滤不打我们的
    机器人 —— 火箭射程远，打对方的机器人既浪费火力、又帮对方减轻基地压力。
...
class Turn(NamedTuple):
    ...
    #: 缺失 ⇒ 空表 ⇒ 无从挑"最值钱的矿"。只读：逐回合抄来的事实。
    vendor_prices: Mapping[str, int] = MappingProxyType({})
    #: 武器商店的价目 `{商品名: 单价}`（顶层 `weaponShopList`）。升级线按它算
    #: "买不买得起" —— 与矿价同一条原则：不写死。查不到的商品按 0 算 ⇒ 买不起 ⇒ 不跑腿。
    shop_prices: Mapping[str, int] = MappingProxyType({})
    #: 判题器本轮报的错。空元组 = 本轮没报错。这是"任务为什么一直失败"唯一的答案来源。
    errors: tuple[Error, ...] = ()
    #: 上回合各实体的动作合法性回执（key 是角色/武器 id、value = 是否合法）。
    ...
    def is_day(self) -> bool:
        """白天吗？`build` 仅白天可用（任务书 §4.4）；`remove` 的昼夜任务书没写，保守起见也仅白天。
    ...
    def day_rounds_left(self) -> int:
        ...
        `planner` 拿它算"这一趟还该采几块石头" —— 按回合计而不记跨回合状态。
        """
    ...
    def summary(self) -> str:
        ...
        不记日志、不留状态、长度有上界（每个列表 `SUMMARY_MAX_ITEMS` 项）；它跑在
        `app.handle` 的 `try` 里，抛出去 ⇒ 整回合退化成空指令 ⇒ 只做字段读取与拼接。
        格式化约定：缺失不等于 0（`-1` 打成 `?`）；射程 -1 ⇒ `?`、≥ `UNLIMITED_RANGE`
        ⇒ `∞`；冷却只在 `> 0` 时出现（-1 与 0 都算"没有冷却"）。
        """
