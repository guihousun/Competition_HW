...
**沉淀的时机**：只要这条经验还没出现在下面的【沉淀的SOP】段里，就应当用 SOP2Prompt 存一条；已经沉淀过的不要重复存。要存的有两类——解题流程，以及探索中拿到的环境知识：接口怎么调、返回什么形状、文件与路径在哪、格式约定是什么（name 写清是什么，流程如「找任务书」、知识如「接口-查询」）。会话窗口只留最近两轮，早先的发现不沉淀就会丢；沉淀过的每一条在后续每一份 prompt 里都看得见。
