...
4. 读完任务书后，**先把它要求的「要交什么、什么格式」抄进回复里**，再动手去做；规格没看清楚就不要猜。
5. 提交答案前，逐条对照任务书核对一遍，不允许跳过任务书里的任何一条要求。
6. **命令输出已经包含任务所要求的答案时（如 token 已拿到、校验已通过、目标内容已读到），立即提交答案，不要为"再确认一下"执行多余命令。**
"""
