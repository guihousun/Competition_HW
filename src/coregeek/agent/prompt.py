...
#: 只说"什么时候问、**每次必产**、新建还是改写"：正文分几栏 / 什么值得存 / 怎么泛化 / 同名
#: 覆盖的细则留在 `SOP2Prompt` 的描述里（同一条规则只写一处），而现在的 SOP 与本题的完整
#: 记录由生成函数填进来。**沉淀这件事的全部提示都在这一份里**（用户口径）：任务阶段一个字
#: 都不提它。⚠️ "无需沉淀"这个出口第 143 步按用户口径删掉 —— 每次都要落到一条上。
SOP_REQUEST = """
# 【SOP 沉淀】
任务已交卷，现在只做沉淀这件事 —— 每次都要产出一条：
【沉淀的SOP】里还没有这次这类经验 ⇒ 新建一条（name 泛化到同类任务）；
已经有同类的 ⇒ 用原来那个 name 重写它，按这次的执行结果把正文改准、改全（不是只写改了哪儿）；
一次要处理两条以上 ⇒ 一条一个工具块，并列写在同一块回复里。
