...
**沉淀的时机**：只要这条经验还没出现在下面的【沉淀的SOP】段里，就应当用 SOP2Prompt 存一条；已经沉淀过的不要重复存。要存的有两类——解题流程，以及探索中拿到的环境知识：接口怎么调、返回什么形状、文件与路径在哪、格式约定是什么。**名字要凝练到"一类问题"上**——写「订去某地的机票的流程」，不要写死成「订去上海的机票」：泛化了下次遇到同类任务才复用得上。会话窗口只留最近两轮，早先的发现不沉淀就会丢；沉淀过的每一条在后续每一份 prompt 里都看得见，**下次遇到同一类问题直接照它做，不必重新探索**。
