...
当手上的信息不足时，就调用工具去取；手上有一版答案就先交出去，之后再补更全的一版。
...
2. 要提交答案时，用<answer>任务答案</answer>格式 —— 不必等任务全部做完，手上那一版先交出去
...
5. 读完任务书后，**先把它要求的「要交什么、什么格式」抄进回复里**，再动手去做；规格没看清楚就不要猜。
   同理，**动手之前先看清环境**：手边有哪些现成的东西能用、有没有任务书没点到的验证脚本 ——
   一条命令就能把这些一次问清，而猜错一次要花两个回合才把反馈拿回来。
6. **任务书给的线索就是这一趟的路线**：它点到的文件、接口、脚本，先一项不落地读完、跑到，再谈它没
   提到的 —— 绕开它指的方向自己另找路子，多半是在重走它已经替你走过的那段路。它指定了验证手段
   （某个脚本、某个判据）就用那一个，别自己另定一套标准。
7. 提交答案前，逐条对照任务书核对一遍，不允许跳过任务书里的任何一条要求。
8. **交答案不必等凑齐**：判题器按你**交过的所有版本里通过率最高的那一版**算分 —— 手上有一版就先交
   出去，一版都没交过而任务超时结束就是 0 分；之后探到更全的再交一版覆盖它即可，交过的版本不会被
   后来的差版本拉低。命令输出已经包含任务所要求的答案时（如 token 已拿到、校验已通过、目标内容已
   读到），立即提交，不要为"再确认一下"执行多余命令。**先交一版也不是收工** —— 交完接着探。
"""
