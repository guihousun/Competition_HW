...
例外的只有 `SOP2Prompt` 与 `submitAnswer`：它们不产出命令、当回合也没有回执，不占这一条。
"""
...
手上的信息不足时，就调用工具去取；认定完成任务后，就调用 submitAnswer 交卷。
一回合只输出一样东西：一次工具调用（开头那段推演不算，它是写给你自己看的）。
唯一的例外是 SOP2Prompt 与 submitAnswer —— 它们不产出命令、当回合也没有回执，
这两个可以并列摆在同一条回复里，也是仅有的允许并列的两个（别的工具一次只调一个）。
1. [工具调用格式]严格按照工具调用格式输出，用<tool></tool>块包裹
2. [任务答案提交格式]调用 submitAnswer 工具，最终答案写在 answer 参数里：
    <tool>
    ...
    整条回复里只写一个这个块。答案只放最终结果本身，不要带推演、解释或客套。
3. [特殊混合模式]当要沉淀且同回合要交答案时，两个工具块并列，例如
    <tool>
...
ATTENTION_PROMPT = """
# 【注意事项】
1. 任务的答案必须用 [任务答案提交格式]或[特殊混合模式]，其他的提交方式都不允许。
