...
信息可信度：实际执行结果 > 实际读到的环境 > 文档描述 > 推测。任务书与文档里写过的东西，在你亲手验过之前就是事实 —— 只有实测回执与它冲突时，才以实测为准。
...
“文档可能写错”是**冲突时**的裁决规则，不是“可以先不读、先不信”的许可。
...
任务书点到的文件、接口、脚本是这一趟的路线，先读完再动手：读一轮就够，试错一轮要两轮。路径、参数名、字段名、取值、格式只能来自**文档原文**或**执行回执** —— 两者都没有就去取，取不到就明说，不许凭印象编。
...
#: 压缩请求的指令 —— 在回合最末尾的压缩闸门发（`prompt` 槽空着就压；交卷轮不压缩）。三槽：
#: 总目标 / 关键数据 / 已完成未完成（第 152 步用户手改：方括号、删掉【下一步】），
#: 加一条输出纪律：只输出一个摘要块。
#: 【关键数据】那句里的"试过并失败的也要列"是试错类任务的记忆 —— 渲染只留「摘要 + 最近一次
#: 工具调用与结果」，失败的尝试掉出尾巴就只能靠摘要兜住。
COMPRESSION_PROMPT = """
# 【上下文压缩】
...
def gen_compression_prompt(material: str) -> str:
    """压缩轮的整份 prompt：独立指令 + **中间段**。

    形状与任务 prompt 同构（标准 messages JSON）。`material` 由 `Context.middle()` 给出：
    题目 + 已有摘要 + 摘要没盖到的全部往来 —— 压缩是在旧摘要之上并进新那段（老账不丢），
    给任务 LLM 的才是压完的。
    """
    return json.dumps(
        [
...
#: 只说"什么时候问、**每次必产**、新建还是改写"，外加**输出定义**（`<sop>` 块的形状与四栏）
#: —— 与压缩请求同一种做法：给一个形状、回来自已解析（`chat.sops_of`），不指望它在长物料
#: 末尾还照工具格式凑一套 XML 调用（第 144 步的起因）。**沉淀这件事的全部提示都在这一份里**
#: （用户口径）：任务阶段一个字都不提它。⚠️ "无需沉淀"这个出口第 143 步按用户口径删掉 ——
#: 每次都要落到一条上。
SOP_REQUEST = """
# 【ROLE定位】
你是一个流程总结大师，擅长通过过往经历沉淀出可复用的经验。你只做经验沉淀这件事，不做其他任何事。

当前任务已完成，对任务进行经验沉淀：
【沉淀的SOP】里还没有这次这类经验 ⇒ 新建一条；
已经有同类的 ⇒ 用原来那个 name 重写它，按这次的执行结果把正文改准、改全（不是只写改了哪儿）；
真的没有新东西时，也要挑最相关的那一条按这次的执行结果核对一遍 —— 对就照原样重写、有偏差就改。

# 【输出格式】
只输出一个 <sop> 块，块外不写任何别的内容、也不要再调用工具。块里要有 <name>这类问题的名字</name>，其余全是正文 —— 两样缺一个，这一条就不作数。

例（**只照形状，正文换成你这次的实际经验**，不要照抄例子里的内容）：
<sop>
<name>订去某地的机票的流程</name>
[适用场景] 需要按出发地、目的地与日期查询并预订机票的任务。
[做法] 先读任务书取出发地/目的地/日期；再查航班列表；取最早一班下单；最后把订单号当答案。
[实测结论] 接口https://example.com的查询接口是 GET /flight/search，参数名是 from / to / date（文档里写的 departure 是错的）；返回里 flights[].no 是航班号。
[注意事项] 日期要补成四位年份，只写月日会被判非法参数。
</sop>

沉淀的是已实际验证、以后同类任务能直接复用的经验（没跑通的东西不进这条）。
name 是这条 SOP 的唯一标识，要泛化到“一类问题”上（例：「订去某地的机票的流程」，不要写成本次的目标）；改已有那条时逐字照抄它原来的 name —— 换个写法就是又存了一条。
正文固定写四栏，没有内容的栏写「无」，一栏起一行：
...
#: 沉淀请求里跟在原始上下文**之后**的那一句（第 144 步）：长物料会把 system 里的格式说明
#: 稀释掉，而离回复最近的那条指令才是模型最认的 —— 所以把"只按格式回块"再说一遍。
SOP_TAIL = """以上是本次任务的全部记录。任务已经结束，现在只做沉淀：按【输出格式】输出 <sop> 块，<name> 与正文都要写全，块外不要再写别的内容、也不要调用工具。"""


def gen_sop_request(sop, material: str) -> str:
    """沉淀阶段的整份 prompt：指令 + 现在的 SOP + 本次任务的完整记录（末尾再钉一遍格式）。
    ...
    第 144 步起不附工具块：形状写在 `SOP_REQUEST` 里、回复由 `chat.sops_of` 解析。
    """
    return json.dumps(
        [
            {
                "role": "system",
                "content": "\n\n".join([SOP_REQUEST.strip(), gen_sop_prompt(sop).strip()]),
            },
...
#: 新闻查价的指令：只进新闻 prompt、不进任务 system —— 没任务时发出去问"官方消息对矿价与
#: 开采的影响"，用任务线之外每游戏日 3 次的额度（`Agent` 指纹带天去重）。回复约定裸 `<prices>`
#: 块（每行 `矿种 方向 起始天 天数`），结构化才能进 `game.core` 那张 10 天表。
#: **输出定义只写在这里**（解析在 `chat.is_prices_reply`、落表在 `core.record_news`）。
NEWS_PROMPT = """# 【市场情报】
读下面的官方消息，判断它对矿产（stone / iron / copper）价格与开采的影响。只输出一个 <prices> 块，每行一条、格式为 `矿种 方向 起始天 天数`：
矿种用英文小写；方向只能是 up（收购价上涨）、down（下跌）、flat（没影响）、stop（采不了）；起始天从明天算起（1 = 明天）；天数 = 影响持续几天。
例：`iron up 1 2` = 明天起连续两天铁价上涨；`iron stop 1 2` = 明天起连续两天铁矿采不了。消息没提到的矿也写一行 `flat 1 1`；某种矿又涨价又采不了就写两行。不要输出任何别的内容。
"""
