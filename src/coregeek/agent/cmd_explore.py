...
2. 清单里的路径**过滤全路径含 `task` 的**（在代码里滤，命令照旧一次拿全 —— 过滤口径要改
   不用动沙盒命令），再逐个 `cat` 落盘到项目根的 `tmp/<原路径>`，读完为止。
...
def _take_list(result: str) -> None:
    """清单回执 ⇒ 建已知路径表与待取队列。一条都没认出来 ⇒ 收工（沙盒没跑起来也是这条路）。

    只认**全路径含 `task` 的** md（沙盒根目录下 md 一堆，这条线只要任务书那几份）；
    ...
    _phase = _FETCH
    LOGGER.info("【沙盒探查】：%d 个 md 带 task：%s", len(paths), " ".join(paths))


def _take_file(result: str) -> None:
    """取文件回执 ⇒ 进日志 + 落盘。落盘失败只告警、继续往下取（任务线那边什么都察觉不到）。"""
    body = result.split("\n", 1)[1] if "\n" in result else result
    LOGGER.info("【沙盒探查】：%s 取回 %d 字：%s", _fetching, len(body), _clip(body, BODY_LOG_MAX))
    ...
    try:
        ...
        LOGGER.info("【沙盒探查】：%s → %s（剩 %d 条）", _fetching, target, len(_pending))
    if not _pending:
