"""Context —— 一个任务的会话上下文，渲染成标准 messages JSON（判题器的 LLM 每回合
只看到 `prompt` 这一段字符串）。这里只管存储与渲染，建会话、每轮放什么由 `Agent` 编排。

存储全量、渲染有窗：消息表逐字全量，`render()` 只输出题目 + 执行摘要 + 最近 `_WINDOW`
轮 —— prompt 是 O(窗口)，长任务的 64KB 沙盒回执堆不出来（无上界增长会撞"响应超时"
红线）。摘要替被窗口掐掉的旧往来记账。`system` 不在消息表里，Agent 每次发送前刷新
（SOP 是活的，任务中沉淀的下一轮就得看得见 —— 那是 `SOP2Prompt` "调用成功"的唯一
回执）。只依赖标准库；措辞在这里、状态在 `Agent`。
"""
...
#: 工具结果用 `tool` 角色（标准 chat 格式的第三个 role）：沙盒回执是工具的产出，
#: 不是人类的指令 —— 标成 `user` 会让判题器的 LLM 把命令输出当成"用户说了什么"，
#: 影响它对"任务是否已完成"的判断（指令性内容归 `user`、工具产出归 `tool`，
#: 不混在一条消息里）。
_TOOL = "tool"
...
#: 渲染窗口（拍的）：原始往来最多保留最近几"轮"—— 一条 assistant 及其后跟着的
#: tool 结果 / 纠错 / nudge 算一轮，更早的只有摘要替它记着。2 的依据：短任务
#: （≤3 轮）掐不着，长任务的 prompt 才真正被压住。要调就改这一个常量。
_WINDOW = 2
...
class Context:
    ...
    def __init__(self, task: str) -> None:
        ...
        self.system = ""
        #: 执行摘要：压缩轮的 `<summary>` 内容（`Agent.adopt_summary` 记入），
        #: 渲染成题目后面的一条 user 消息，替被窗口掐掉的旧往来记账 —— 所以它必须
        #: 装得下"要交什么 + 关键数据原文"（压缩指令的四槽结构管这个）。
        self.summary = ""
    ...
    def feed(self, result: str = "", retry: str = "") -> None:
        """回灌轮的新消息（按 role 分条）：沙盒结果 = `tool`、纠错 = `user` —— 标题
        留在 content 里当内容标签（沙盒输出是任意文本，没标签分不清哪段是什么）。
        结果单独占一条 `tool` 消息：命令输出是工具的产出、不是人类指令（见 `_TOOL`）。
        """
        if result:
    ...
    def tool_output(self, text: str) -> None:
        """本地工具（`python_exec`）的产出：一条 `tool` 消息，当场进表 —— 它跟在
        `hear` 记下的那条 assistant 调用后面，下一份 prompt 窗口里就能看到。
        与沙盒回执同一条纪律（工具的产出归 `tool` 角色），标题标明来源 —— LLM
        才分得清"这是本地算的"与"这是判题器沙盒跑的"。"""
    ...
    def render(self) -> str:
        ...
        题目永远完整（"要交什么"的权威来源，掐什么也不能掐它）；摘要在题目后面、
        原始往来前面 —— 它讲的是旧账，最近的往来才是现状。`json.dumps`
        （`ensure_ascii=False`、紧凑分隔符）—— 正文里的 `{}`、换行、标签逐字保留。
        """
        messages = [
    ...
    def _window(self) -> list[Message]:
        ...
        render 单独放），最后 `_WINDOW` 条 assistant 里最早的那条起、其后全部
    ...
    def material(self) -> str:
        ...
        原文永久保留、压缩总从原文重来（不从旧摘要叠 —— 避免多次压缩的失真累积）；
        消息表永不截断，代价是压缩请求随历史线性变大（任务 prompt 是有界的）。
        与 `render` 的分工：render 给任务 LLM 的是压缩后的（摘要 + 窗口），
        这里给压缩器的是原文。
        """
        return json.dumps(
