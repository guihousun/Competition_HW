...
`_sop`（流程表 `{流程名: 正文}`，整场存活）、`_news_digest`/`_price_hints`（新闻指纹与
价格期望）、`_context`（任务内会话，题目变了即换新）。失败退化路径：状态丢了只影响
prompt 的内容、不碰红线（会话丢了 ⇒ 退化成单轮提问）；判题器逐回合同步请求 ⇒ 不加锁。
SOP 不按任务分区（任务 A 的沉淀会灌进任务 B）；会话按任务分区（身份 = 题目原文）。
`planner.task_channel` 每次都用包根那个 `AGENT`，不能每回合新建。
"""
...
class Agent:
    ...
    def __init__(self) -> None:
        ...
        #: 价格期望与新闻指纹。退化路径：期望错了 = 采矿偏好偏一天，不碰红线；
        #: 指纹丢了 = 重问一次新闻（额度 3/日）。
        self._news_digest = ""
        ...
        #: 描述与参数说明是 prompt 的一部分（`prompt.gen_all_tool_prompt` 生成），措辞
        #: 直接决定调用正确率；参数表同时是 `tool_call` 的调度签名 —— 一张表两处用，
        #: 描述与调度不会分家。顺序即 prompt 里的顺序。表必须由实例构造：
        #: `SOP2Prompt` 写的是 `self._sop` ⇒ 只能是绑定方法。
        self._tools: dict[
    ...
    def chat(self, request: str, *, result: str = "", retry: str = "") -> str:
        """组装累积的会话发给判题器的 LLM。`request` = 题目原文；同一道题续上旧会话、
        换题换新。首问 = 题目（构造即问）；回灌轮 = 结果/纠错（`feed`）；无新内容的重问轮
        = 一句「请继续。」（`nudge`）—— 判题器的 LLM 是黑盒，会话停在它自己的输出上
        是个含糊指令。返回值是标准 messages JSON（`[{role, content}]`）。
    ...
    def adopt_summary(self, text: str) -> None:
        ...
        摘要进 `Context.summary`、回复不进会话表 —— 它不是 LLM 在任务上说过的话。
        """
        if self._context is not None:
            self._context.summary = text

    def compression_request(self) -> str:
        """压缩轮的 prompt：独立指令 + 原始上下文全文。

        发送时机 = 回合最末尾的压缩闸门：`task_channel` 判据链算完 `prompt` 还是空
        （只剩命令轮 —— 答案轮不压缩，压缩与 `<answer>` 互斥）才发，模型请求永远优先。
        任务期间 prompt 不限量不计数。没开过会话 ⇒ `""`。原料由 `Context.material()`
        给出：原文永久保留、压缩总从原文重来；给任务 LLM 的才是压缩后的（摘要 + 窗口）。
        """
    ...
    def python_exec(self, code: str) -> str:
        """本地即时计算：`pyexec.run` 跑代码，产出当场记进会话（`tool` 消息，跟着
        LLM 那条调用走），返回 `""` —— 不产命令。重问的 prompt 窗口里它看得见自己
        的调用与产出，下一回合就能作答，比 `executeCmd` 省一整个沙盒往返。

        与 `executeCmd` 的分工（两边的描述里都写了）：那是判题器沙盒（能看任务文件、
        一回合往返、限 15 秒）；这里是我们进程里的纯计算（即时、看不见沙盒、
        安检 + 2 秒超时）。没开会话时产出丢弃（任务已结束的迟到回复，无害）。
        """
    ...
    def news_question(self, news: str) -> str:
        """没任务时的新闻查价 prompt。同一份 news 只问一次（指纹去重 —— 任务线之外
        每游戏日只有 3 次额度）；news 空 / 指纹没变 ⇒ `""`。指纹在发问时就记下：
        判题器不答也只是"不再问了"，不重发同一份。
        """
        if not news:
            return ""
    ...
    def adopt_price_hints(self, hints: dict[str, str]) -> None:
        ...
        粗粒度方向：up ×2 / down ×0.5 / flat ×1 —— 只修正挖矿性价比的排序，
    ...
    def tool_call(self, tool_name: str, params: list[tuple[str, str]]) -> str:
        ...
        `params` 是 `tool_of` 解析出来的 `[(参数名, 原文), …]`，只收具名参数：带名的
        按名对，认不出的名字忽略（不为一个编造的名字作废整次调用）；声明了的参数
        一个不少、值是非空字符串才放行，最后 `impl(**resolved)` —— 无参数工具
        就是 `impl()`。

        "返回值即命令"是铁律：`""` = 这个工具不产出命令（`SOP2Prompt`）/ 调用不成立
        （未知工具、形状不对、缺参数、空白值），下游不需要区分，全落到重问。
        绝不抛异常（它跑在 `app.handle` 的 `try` 里，抛出去会把整回合所有角色的指令
        一起带走）⇒ 一切不成立都返回 `""`。边界校验只在这一处：这是唯一一个由
    ...
    def SOP2Prompt(self, name: str, sop: str) -> str:
        """把一条条目（`name` = 条目名、`sop` = 流程总结或环境知识）沉淀进流程表。
        返回 `""` —— 不产出命令。

        条目有两类：流程（怎么做）与知识（接口怎么调、路径在哪、格式是什么），
        同一张表、类型由 `name` 约定区分（知识条目建议「接口-XX」这类名）；存储、
        渲染、上限对两者一致。存储规则（单条上限、条数上限、同名覆盖、截断留痕、
        内容没变就静默）在 `tools/sop.py`，这里只管把新表记在自己身上；方法名
        同时是注册表里的工具名。

        答案不归这个工具管：同轮作答交给 `chat.answer_of`（答案写在工具块外的
        `<answer>` 里）—— 这一回合算不算作答由那个谓词判，两条都走不通就落重问，
        而沉淀已经落库、丢的只是那一回合。
        `sop` 里成对的 `<answer>…</answer>` 在入库前挖掉（`chat.strip_answers`）：
        那段正文讲的往往正是"答案怎么写"，不挖掉会带着这对串进后续每一份 prompt。
        挖掉、不是作废整次调用 —— 沉淀是这个工具的全部价值。空文本 = 删掉那条
        （只在直接调用时可达：`tool_call` 的闸门挡空白）。
        """
    ...
    def reset(self) -> None:
        """清空全部跨回合状态（流程表、新闻指纹、价格期望、会话）。只给用例用 ——
        单实例是模块级的，同一个测试进程里会跨用例串味。
