...
命令在沙盒里可能失败，而回执是下一回合唯一的线索：让命令自带成败证据 —— 关键步骤回显输入与结果，成功也要看得见（静默的回执什么都证明不了），失败要把原因打全（错误原文、出错的文件与行号）。
Python 代码用 try/except 包住可能出错的部分，在 except 里打印完整调用栈（traceback.print_exc()）—— 光看退出码说明不了错在哪一步。
Python 代码里调 shell 命令时，shell 的原始输出必须原样打印出来，不能只打印加工后的结论 —— 结论漏了重点，这条命令的两个回合就白花了。
