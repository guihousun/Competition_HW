...
这是高成本工具：一次调用消耗 2 个回合 —— 能在一次命令里完成的连续操作应尽量合并，只有后续操作依赖当前输出（无法安全合并）时才拆分；多个候选值之间试错，也在一次命令里批量尝试。

回执是下一回合唯一的线索，每条命令都得让它自己把话说清楚：每次尝试给清晰标签分家；关键步骤回显输入与结果 —— 让命令自带成败证据，成功也要看得见（静默的回执什么都证明不了），失败要把原因打全（错误原文、出错的文件与行号）。

shell 指令的执行结果会原样回到回执里 —— 不做计算就直接发 shell 命令，不必再套一层 Python。要计算（过滤、统计、格式转换）才用它：Python 代码用 try/except 包住可能出错的部分，在 except 里打印完整调用栈（traceback.print_exc()），光看退出码说明不了错在哪一步；用了 shell 就把它的原始输出一并打出来，别只打加工后的结论。
...
多写的字直接扣分。被判错时判题器会给出反馈，按反馈改了再交一次。""",
    ...
    def compression_request(self) -> str:
        """压缩轮的 prompt：独立指令 + **中间段**（`Context.middle`：题目 + 已有摘要 +
        覆盖点之后的全部往来）。没开会话、或没有可压的（中间段空）⇒ `""`。

        发送时机 = 回合末尾的压缩闸门：那一轮 `prompt` 槽没被别的东西占住（命令轮与它搭同
        一轮，回复搭下一轮那个本来空着的 `llmResp` 槽 ⇒ 不额外花回合）。这里顺手给上下文记
        一个快照（`sent_for_compression`）：摘要回来时按它划覆盖点 —— 判题器不答的话快照
        不动，那些往来照旧全量渲染、下一个空槽照旧当中间段压一遍（**不丢**优先于不重问）。"""
        if self._context is None:
            return ""
        material = self._context.middle()
        if not material:
            return ""
        self._context.sent_for_compression()
        return gen_compression_prompt(material)
