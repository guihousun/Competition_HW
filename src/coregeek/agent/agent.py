...
[适用场景]当目标文件已经出现在本工具描述中的[路径]里，并且当前只需要读取其内容时，优先使用本工具。
...
[重要说明]
1. [路径清单]之外的文件不会由本工具主动搜索。
2. [路径清单]中暂时没有某个文件，不代表沙盒中不存在该文件。
3. 如果目标文件不在[路径清单]中，使用 executeCmd 查找并读取。
4. 沙盒知识[路径清单]会随着环境探查逐步增加。
