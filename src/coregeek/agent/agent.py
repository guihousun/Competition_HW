...
class Agent:
    ...
    def compression_request(self) -> str:
        ...
        发送时机（第 43 步改定；第 47 步收窄）：**回合最末尾的压缩闸门** ——
        `task_channel` 判据链算完 `prompt` 还是空（只剩 ③ 命令轮 —— 答案轮不压缩，
        压缩与 `<answer>` 互斥）才发；判据 ②/④/⑥ 的模型请求永远优先。
    ...
    def SOP2Prompt(self, name: str, sop: str) -> str:
        """把**一条**条目（`name` = 条目名、`sop` = 流程总结或环境知识）沉淀进流程表。
        返回 `""` —— **它不产出命令**。

        第 46 步起条目有两种类型（用户口径"探索到的环境知识也要沉淀为后续指导"）：
        **流程**（怎么做）与**知识**（接口怎么调、路径在哪、格式是什么）—— 同一张表、
        类型由 `name` 约定区分（知识条目建议「接口-XX」这类名），零新机制：存储、渲染、
        上限对两者完全一致（知识描述可能较长，`SOP_MAX` 截断留痕兜着）。

        存储规则（单条上限、条数上限、同名覆盖、截断留痕、内容没变就静默）在
        `tools/sop.py`，这里只管**把新表记在自己身上**。方法名同时是注册表里的工具名。

        ⚠️ **答案不归这个工具管**（第 36 步的用户口径传承）：同轮作答交给
        `chat.answer_of`（答案写在工具块外的 `<answer>` 里）⇒ 沉淀与作答分家：
