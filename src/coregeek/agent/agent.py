...
比 executeCmd 省一个回合（它要 2 个回合、还要等沙盒回执）：不需要访问沙盒的活儿一律用它，不要占用昂贵的沙盒执行。
            ...
            "SOP2Prompt": (
                self.SOP2Prompt,
                """
把当前任务中已实际验证、以后同类任务能直接复用的知识沉淀成 SOP。
只要这条经验还没出现在【沉淀的SOP】里，就存一条；已经沉淀过的不要重复存，用同一个 name 更新它。
该存的有两类：这一类任务的通用做法（去哪儿取信息、按什么顺序做、结果怎么用），
以及实测确认过的环境知识 —— 接口真实路径、参数名与取值、返回值结构、文件位置、工具行为、文档与实测的差异。
