"""Agent 的"说什么 / 怎么读回复"：prompt 的四段模板（system header）+ 三个谓词。
...
（`Context`）、编排在 `Agent.chat`；这里只剩**常驻说明**（模板）与**回复解析**（谓词）
—— 两者不拆成两个模块：它们是同一份约定的两面，拆开就是"改一处要翻两个文件"。

⚠️ **协议形状是我们自己定的**（任务书只说了沙盒能跑什么，没规定 LLM 该怎么要命令）
⇒ `# 输出格式` 那段必须把两个形状逐字给全，并保留旧形状兼容与"原文即答案"的兜底。
"""

#: 发给判题器 LLM 的 **system header**。四段（**定位** → **工具** → **输出格式** →
#: **沉淀的 SOP**），后面由 `Context.render` 接「# 对话记录」与这道题的全部往来。
#:
