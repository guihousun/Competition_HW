...
（`planner.task_channel`），"怎么解析回复"与战场规则无关 ⇒ 本模块是纯的、零包内 import。
`strip_answers` 同住，因为它认的就是 `_ANSWER_RE` 那一个模式，分家等于把标签语法抄成两份。
协议形状是我们自己定的（任务书只说了沙盒能跑什么，没规定 LLM 该怎么要命令）⇒ 严格只认
嵌套形状：`prompt.py` 教什么、这里就只认什么；其他形状靠 `looks_like_tool` 判宽接住、落
重问（丢一回合、不碰红线）。
"""
...
#: "成对才作数"是它们的共同要求（半截绝不能当内容用 —— 要么被当答案交上去、要么被当命令发
#: 进沙盒），所以每个模式都自带 `</…>`；非贪婪 `.*?` 停在第一对上（一回合只跑得了一条命令）。
#: 开标签一律写成 `<tool\b[^>]*>` —— 按 `>` 的位置定位、不是逐字匹配整段标签（多敲一个空格、
#: 加个属性都不该让整条回复作废）；`\b` 挡的是 `<tool_name>`／`<tool_param>` 被当成工具块的开头。
_TOOL_RE = re.compile(r"<tool\b[^>]*>(.*?)</tool>", re.DOTALL)
_NAME_RE = re.compile(r"<tool_name\b[^>]*>(.*?)</tool_name>", re.DOTALL)
#: 嵌套形状：参数写在 `<tool_param>` 里面再包一层的具名标签 —— `<tool_param><cmd>ls</cmd>
#: </tool_param>`。开标签按 `>` 定位（`<cmd >` 也认），闭标签用反向引用 `\1` 认同名那一对；
...
#: 反转义表：`prompt.TOOL_PROMPT` 教了 LLM 对 XML 特殊字符转义 ⇒ 参数值里的五个预定义实体
#: 要还原。`&amp;` 必须最后换（`&amp;lt;` 只该还原一层）。LLM 没转义时这条是空操作 ——
...
def strip_answers(text: str) -> tuple[str, int]:
    ...
    唯一的调用者是 `Agent.SOP2Prompt`：SOP 正文里不许有这对串（沉淀的正文讲的往往正是
    "答案要用标签包"，与其要求 LLM 每次绕开，不如在入库这唯一一道口子上保证存下来的那份
    没有）。挖掉、不是作废整次调用 —— 沉淀是那个工具的全部价值；挖了几处进日志。只认成对：
    半截标记原样留着（`answer_of` 认的也是成对块），空块照挖。
    """
...
def tool_of(reply: str) -> tuple[str, list[tuple[str, str]]] | None:
    ...
    2. `<tool_name>` 取第一块、`<tool_param>` 全部收集，每块里必须有一个或多个具名元素
       （属性式 `name="…"`、裸值都不认）；任何一块里一个都没有 ⇒ 整次调用 `None`。参数值只
       去首尾空白（内部换行保留），再过 `_unescape`。
    3. 只有参数没有名字 ⇒ `None`（不猜工具名）；只有名字没有参数是合法形状 —— 放不放行由
       `tool_call` 按声明的参数表判。块是空的 ⇒ `None`。只取第一条 `<tool>` 块。
    """
...
def _params(body: str) -> list[tuple[str, str]] | None:
    r"""收集全部 `<tool_param>` 块里的具名元素 ⇒ `[(参数名, 原文), …]`；任何一块里一个具名
    元素都没有 ⇒ `None`（整次调用不成立）。

    元素名取标签名（`\w+`，中文也认），值只去首尾空白（多行命令、带缩进的 python 都合法）。
    参数可拆进多个块（块数不是判据）。半截的内层标签配不上 `\1` ⇒ 那块算"没有具名元素" ——
...
def summary_of(reply: str) -> str:
    ...
    best-effort：摘要取不到（没写 / 半截 / 空块）⇒ `""`，调用方保留旧摘要继续 —— 绝不因为
    摘要缺失而重问（摘要是搭车品、重问要花一回合）。不回落原文：拿半截标记去凑只会把标签串
    当内容用。
    """
...
def is_summary_reply(reply: str) -> str | None:
    ...
    判据：`<summary>` 块取得出内容，且挖掉摘要块之后一个字都不剩 —— 任务回复（带工具调用 /
    答案 / 正文）不算。任务 prompt 不教摘要 ⇒ "裸摘要"几乎必属压缩回复；粘住同文再判仍是幂等。
    """
...
def is_prices_reply(reply: str) -> dict[str, str] | None:
    ...
    判据与 `is_summary_reply` 同构：块取得出内容、挖掉之后一个字不剩。行格式 `矿种: 方向`
    （大小写都认）；解析不出的行直接丢 —— 宁可少认一条，不猜。
    """
...
def looks_like_tool(reply: str) -> bool:
    ...
    判得比 `tool_of` 宽是有意的：只有 `<tool_name>` 的回复、解析不出的形状（属性式 / 裸参数 /
    裸工具块）都该被认成"它想调工具、但格式没凑对" ⇒ 落到重问，而不是被当成答案交上去。
    代价是答案里含字面量 `<tool` 会被误判（概率极低，降级方向安全）。本模块唯一只认前缀的
    地方 —— 别顺手改成 `_TOOL_RE`：那会把"想调但没凑对"的回复从重问变成"原文即答案"。
    """
...
def answer_of(reply: str) -> str:
    ...
    上一回合真正交上去的那一份 ⇒ 两处各判一次就会出现"拿着 `<answer>晴</answer>` 去骂'你
    上次答的 `晴` 不对'"。

    三级判据：

    1. 把全部结构块（每一对 `<tool>` 与 `<summary>`）整段挖掉得 `rest`，再扫：出现 `<answer`
       标记 ⇒ 只认成对块的内容（配对不上或为空 ⇒ `""`，不回落原文）。先挖结构块防的是污染：
       SOP 正文与执行摘要讲的往往正是"答案要用 `<answer>` 包"，里面就有字面量实例；落在
       结构块内的 `<answer>` 一律不算答案（分不清是真是假，就不许当成答案交上去）。
    2. 否则像是工具调用 ⇒ `""`（用原文判 —— 挖完就不像了，会误放行 `<tool>ls</tool>` 后面
       跟着的那句话）。
