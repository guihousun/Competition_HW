...
5. **每回合的复盘日志（`app._log`）写在 `try` 里面，别挪出去。** 日志代码再不起眼也是代码，
   ...
   | 有回执 | 43 | 2453 | 约 26 回合 |
   | 任务在身、还没答过（提问那一轮，`prompt` 全文 ≈3KB） | 65 | 5907 | 约 11 回合 |
   ...
   不砍现场。真出事只能把 `LOG_TEXT_MAX` / `LOG_PROMPT_MAX` 调小或者少打一块。
...
│   └── actions.py    BaseAction + 各动作（`move` / `build` / `collect` / `attack` / `acceptTask` / `submitAnswer`）。**创建即校验**，唯一懂线上动作格式的地方（`to_wire` 写、`describe` 读）。`describe` 第 20 步起**通用摊开**（有什么字段打什么，字符串值过 `app` 递进来的 `clip`），不再维护字段清单
...
`tests/` 只有 `test_actions.py` 一个文件（权限 / 报文 / 几何 / 决策 / 解析五类），**不建自研测试框架**：标准库 `unittest` 够用。**198 条**。
进度见 `docs/design/code-task.md`（当前到第 20 步：**日志打详细** —— 动作行改成
...
字节（约 10 → **7 回合**写满 64KB，用户拍板"接受上升、重新钉上限"，守卫上限 6900 → 9300）；
