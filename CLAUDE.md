...
5. **每回合的复盘日志（`app._log`）写在 `try` 里面，别挪出去。** 日志代码再不起眼也是代码，
   ...
   ⚠️ **stdout 有被写满的风险**（第 16 步在真服务上重测，含时间戳前缀）：

   | 局面 | 行 | 字节 | 写满 64KB |
   |---|---|---|---|
   | 干净回合 | 41 | 2424 | 约 27 回合 |
   | 有回执 | 43 | 2537 | 约 25 回合 |
   | 任务在身（两个字段顶到 `LOG_TEXT_MAX`=400） | 42 | 4893 | 约 13 回合 |
   | 任务在身 + 沙盒结果顶格 | 43 | **6153** | 约 **10 回合** |
...
- **任务线的三个权威事实**（第 11 步，全部交叉验证过）：
  ...
  - ⚠️ **沙盒是任务线的另一半，第 16 步之前整条都没接**（那时 `executeCmd` 恒为 `""` ⇒ 答案只能靠 LLM 硬猜 ⇒ 判题器只能报 `errorCode 2`，**这就是"任务一直失败"的根因**）。四个事实：
    ...
    - **`<tool>…</tool>` 是我们自己定的工具协议** —— 文档只写了沙盒"能执行基础的 shell 指令与 python 指令、**无法访问外部网络**"（任务书 L439），**没规定 LLM 该用什么形状要命令**。协议形状是本步最大的单点风险，只能靠实盘检验。
    - **`planner.task_channel(turn) -> (prompt, executeCmd)`** 是这两条通道**唯一**的产出点，判据五条、从上往下先命中先返回：① 没任务**或名册里没有开拓者** ⇒ 两个都不发；② `cmd_result` 非空 ⇒ 回灌结果、**这轮绝不发命令**；③ 回复里有 `<tool>` ⇒ 发命令、不提问；④ 判题器报 `code 2` ⇒ 带"上次答错了"重问；⑤ 回复是最终答案 ⇒ 都不发；⑥ 否则只把题目问出去。
      ...
      **纠错（④）是"修饰符"不是"分支"**：`errors` 说的是**本轮产生**的错误，与我们上轮发了什么**不同步**（发命令那轮没有 prompt，下一轮却可能同时拿到沙盒结果和更早那次 `submitAnswer` 的判决）—— 做成独立分支就会把纠错整段吞掉。触发条件是 `code == 2` **且**回复非空**且**回复不是工具调用（后两条挡住"任务刚换、`errors` 里是旧账"和"骂一条命令像答案"）。
      **畸形工具回复**（`<tool` 有开无闭）落到判据 ⑥ 重问，**不能**当成答案 —— 判据 ⑤ 与 `_answer_task` 用**同一个谓词** `_is_tool_reply`（判 `"<tool" in reply`，故意判宽）；两边判据一旦分家，就会出现"提问与提交同时哑火、永久空转且日志上什么都看不出来"。
...
- **复盘日志的体量与格式**（第 12 步，第 14 步扩了触发条件）：`Turn.summary()` 的每个列表**必须有上界**（`SUMMARY_MAX_ITEMS`）—— `Turn.robots` 是逐回合全量、没有上界，日志长度不能是"数据相关的量"。摘要里 **-1 是"字段缺失"不是 0**（金币/回合号打成 `?`）。三块（摘要/图例/地图）拼成**一条**日志记录 —— `logging` 的时间戳只加在第一条物理行上，拆开地图那几十行就没时间戳了。
  ...
  **沙盒行只记结果、不记发出去的命令**：发命令那一回合 `llm_resp` 就是那条 `<tool>…</tool>`，已经印在任务行的"提交："里了，再抄一遍是**同一回合、同一条字符串抄第二遍**（与"`prompt` 不打印原文"逐字同源）；而且"沙盒行数 = 实际跑过的命令数"这个对账关系更干净。
  ...
  **`prompt` 不打原文**：它就是 `TASK_PROMPT` 套上任务原文（第 16 步起还可能带上沙盒结果与纠错那两段），全文等于把任务抄第二遍，有意义的只是"这回合到底问没问"。
...
`tests/` 只有 `test_actions.py` 一个文件（权限 / 报文 / 几何 / 决策 / 解析五类），**不建自研测试框架**：标准库 `unittest` 够用。**151 条**。
进度见 `docs/design/code-task.md`（当前到第 17 步：**围墙只砌三面**（14 格，背面整列留成"门"）
\+ **不能把工人关起来**的闸门（`_trapped` / `_walled` / `step_outside`；
e2e 还顺带揪出并修掉了"人站在墙上时对着脚下同一格反复 `build`"的死循环 —— `_step_aside`）；
第 16 步是给任务线接上沙盒（响应顶层 `executeCmd` 与 payload 顶层 `lastCmdResult`
靠 `<tool>…</tool>` 协议 + 结果原文回灌 + "答案错误"时的纠错回路串成闭环，
`planner.task_channel(turn) -> (prompt, executeCmd)`）；
