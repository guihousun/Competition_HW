...
5. **每回合的复盘日志（`app._log`）写在 `try` 里面，别挪出去。** 日志代码再不起眼也是代码，
   ...
   | 局面 | 行 | 字节 | 写满 64KB |
   |---|---|---|---|
   | 干净回合 | 46 | 2479 | 约 26 回合 |
   | 有回执 | 48 | 2621 | 约 25 回合 |
   | 任务在身、还没答过（提问那一轮，`prompt` 全文 ≈3KB） | 70 | 6056 | 约 10 回合 |
   ...
   不砍现场。第 22 步的 banner（`handle` 打的 `###第N回合###`）+ 摘要改成带空行的四块
   + 角色行加打铁/铜，又把最坏抬到 **71 行 / 8723 字节**（上限仍是 9300，
   余量 577 仍小于"抓结构性膨胀"所需的一千二百字节）。真出事只能把
...
└── game/             领域与策略
    ...
    │                 + `within` / `is_day` / `day_rounds_left` / **`summary()`（四个带空行的块）**
...
⚠️ `utils` 是**叶子**（一个包内 import 都没有），目前只有 `app` 取它。
...
- **地图字符表：空地是「空格」、我方围墙 `#`、敌方围墙 `%`**（**第 22 步定案**：用户手改 + 本步做完）。空地的历史是"`.` → 铺满 `#` → 空格"三段：第 21 步按"打满整个矩阵比较显著"把空地铺成 `#`、墙让位给 `=`；随后用户把空地的填充撤掉改成**空格**，`#` 空出来**收回给墙**。⚠️ **第 21 步已知不确定性 #2 当天就兑现了**（"满屏 `#` 之后背景反而比结构显眼"）—— 这才是撤销的动因。"这张图有多大、边界在哪"现在由**行号槽 + 两行列标尺**负责（`render()` 里本来就有，不靠填满）。两条不变量照旧：**① `_char` 恒返回 1 个字符**（全角字符占 2 个显示列，一引入就整图错位）；**② 一个字符只能代表一样东西** —— 守门员 `assertEqual(len(set(chars)), len(chars))` 是**通用断言**（钉的是"不重复"，不是具体字符值），字符怎么改它都守着。⚠️ 图例里那一项写的是字面量 `空格=空地` 而不是 `f"{_char('')}=空地"`：空地就是一个空格，直接拼出来是 `" =空地"`，看着像少打了一个字符。字符变化是 1 字节对 1 字节 ⇒ 地图自身的体量不变（第 22 步那张字节表的变化来自 banner/摘要，与字符表无关）。
...
- **复盘日志的体量与格式**（第 12 步，第 14 步扩了触发条件）：`Turn.summary()` 的每个列表**必须有上界**（`SUMMARY_MAX_ITEMS`）—— `Turn.robots` 是逐回合全量、没有上界，日志长度不能是"数据相关的量"。摘要里 **-1 是"字段缺失"不是 0**（金币/回合号打成 `?`）。三块（摘要/图例/地图）拼成**一条**日志记录 —— `logging` 的时间戳只加在第一条物理行上，拆开地图那几十行就没时间戳了。
  **摘要从第 22 步起是四个块、块间空一行**（`【回合】`/`【我方】`/`【机器】`/`【可接任务点】`），
  我方那一行还打出 `石N铁N铜N`（"工人有铜却不去卖"只有背包明细能回答）；
  ⚠️ **空行也是物理行** ⇒ "摘要给日志定序"那条对账关系从"数行"变成"**数非空行**"。
  **记录数不固定，每条都"有事才吭声"**：局面、动作每回合各一条；**判题器报错 / 回执名单 / 任务线 / 沙盒回执**四条各自只在有内容时出现（干净的白天回合 2 条，样例那种带假错误的局面 4 条）。`errors` / `action_results` / `cmd_result` 的空值是**天然的安全值**，所以正常局面一个字节都不花 —— 这是它们能与硬约束 5 共处的原因。
  ⚠️ **`handle` 在 `_log` 之前还打一条 banner**（`###第N回合###`，用户加的）——
  它**不受 `_log` 两个上限管**，数"这一回合打了几条记录"时要单独加上（干净回合 3 条不是 2 条）。
  ⚠️ **"任务行 / 沙盒行"曾被误删过一次**（第 22 步恢复）：那两块是黑盒下**唯一**能看见
  `prompt` 全文与沙盒回执的地方（本地 e2e 的"LLM"是我们自己写的，只证明解析自洽）⇒ 动 `_log` 时别顺手删。
  **沙盒行只记结果、不记发出去的命令**：发命令那一回合 `llm_resp` 就是那次工具调用（新旧形状都算），已经印在任务行的"提交："里了，再抄一遍是**同一回合、同一条字符串抄第二遍**；而且"沙盒行数 = 实际跑过的命令数"这个对账关系更干净（第 18 步 e2e 实测过：九轮里只有一轮带 `lastCmdResult` ⇒ 沙盒行恰好 1 条）。
  ...
  **`prompt` 打全文**（第 20 步改，原来是"有（N 字）"）：它 = 模板 + 沉淀的 SOP + 沙盒结果 + 纠错 + 题目，**别处看不见** —— 工具清单长什么样、SOP 那个槽填进去没有、题目在不在里面，实盘上只有这一行能回答（本地 e2e 的"LLM"是我们自己写的）。
...
进度见 `docs/design/code-task.md`（当前到第 22 步：**卖矿线 `sell`** —— 工人墙砌完之后
...
顺带三件事：`_clip` 搬进新叶子模块 **`utils.py`**、摘要改成**四个带空行的块**并打出 `石N铁N铜N`
（最坏回合 → **71 行 / 8723 字节**，上限仍是 9300）、**地图字符表反过来**
