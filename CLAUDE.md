...
`tests/` 只有 `test_actions.py` 一个文件（权限 / 报文 / 几何 / 决策 / 解析五类），**不建自研测试框架**：标准库 `unittest` 够用。**135 条**。
进度见 `docs/design/code-task.md`（当前到第 16 步：**给任务线接上沙盒** —— 响应顶层 `executeCmd`
与 payload 顶层 `lastCmdResult` 这两条一直没接的通道，靠 `<tool>…</tool>` 工具协议 + 结果原文回灌 +
判题器报"答案错误"时的纠错回路串成闭环；`planner.prompt_for` 随之换成
`task_channel(turn) -> (prompt, executeCmd)`）；
