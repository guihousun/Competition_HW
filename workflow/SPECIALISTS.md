...
2026-09-14 用户更新：Codex可按任务适配度直接完成架构、复杂定位、关键修复及其他适合亲自
处理的工作，不再为额度强制派单。范围清楚的实施仍可复用DSH；同一文件避免并发修改。
已经运行的DSH任务继续等待或原生插入调整，不因等待器超时重复派单、重启或接管。

缺少被测SHA或完整日志时，明确证据缺口和已知开发基线，继续可独立验收的诊断、工具和修复。
缺少证据只阻断其依赖步骤与需要同SHA内网PASS的合并，不把整个Issue停在等待用户定位。
