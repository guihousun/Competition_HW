...
- `awaiting-intranet`：已审核候选整合到 `codex/sgh`，附被测 SHA、参赛包和回测步骤，等待真实反馈；不自动关闭原 Issue。
- `merge-ready`：测试分支整合需当前 head 审核及本地检查通过、无未解决意见；官方行为的正式稳定版本晋升需用户对应 SHA 内网 PASS。
- `merged`：Codex核对目标分支与预期 SHA 后整合，禁止强推、绕过分支保护或自行增加权限；整合状态与内网验证状态分开记录。
- `verified`：有足够证据后再关闭Issue。新反馈失败时回到分诊，保留历史证据。

只允许自动合并本工作流创建且已审查的PR，不批量合并仓库其他PR。
`codex/sgh` 是固定测试集成分支，经过审核和本地检查的候选可先整合供内网测试。正式稳定版本晋升或官方通过声明仍需实际被测 SHA 的内网 PASS。
...
`--timeout`只约束调用方等待（观察者），不设DSH执行期限：等待超时退出码2，不发送shutdown、不kill进程、
不取消当前轮、不标记failed、不重新派单；用同一输出目录再次 `wait` 即可接收迟到结果。
native轮次超时/失去真实进程的原因分别记录：进程消失报transport loss，轮次仍在跑只报“仍在运行”。
Manifest是流程约束，不是签名/操作系统沙箱；更强隔离应另配容器或独立系统账号。

worker失败/停止后不得再用 `send` 顶替：该命令会明确拒绝。用 `resume` 走DSH原生恢复
（`ctx.agents.resume`，保留原session ID与原有上下文），前置条件、证据校验和确认输出见 `STEERING.md`。
`resume` 的排队回执是 `resume_requested: true`，只有worker写入 `native-resume.json`/`native_resumed` 才算原生已恢复。

6. Codex比对保护文件、允许改动路径和Git HEAD。dsh越界修改或自行提交即拒绝自动合并，保留现场。
7. 用当前代码实际执行测试，自己阅读结果，不只复述dsh的“测试通过”。
8. 审核及相关本地检查通过后，由 Codex 整合并推送固定测试分支 `codex/sgh`，管理内部 PR 并生成同 SHA 参赛包。用户始终拉取 `codex/sgh`，无需切换每个工作分支。此分支允许等待内网验证的候选；正式稳定版本晋升与官方通过声明仍须真实内网证据。
...
候选修复→审核并整合测试分支→公司复测→正式稳定版本晋升的验证结论须逐项登记，不从分支整合或本地测试推断内网 PASS。
