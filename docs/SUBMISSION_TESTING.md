...
**2026-09-14 打包兼容背景**：Issue #10 报告 `cant get package`，owner 说明是**直接上传压缩包**（不是下载 URL）。
...
以已经审核、通过相关检查的独立变更为交付单位，不等待所有 DSH 探索、策略研究或关联 Issue 全部结束。每次候选更新固定分支和根目录包，并在来源 Issue 留下源码 SHA、交付 SHA、包 SHA256、具体行为变化、验证范围与回测重点。公司侧 Agent 据此拉取测试并回填真实结果；这里只能发布回测任务，不能代替公司环境执行或把本地成功当作内网成功。

交付要明确区分参赛策略、模拟器规则实现和实验工具。只改模拟器或文档的版本应说明正式策略是否变化，公司侧可继续提供真实行为证据用于校准，不把同一策略的重打包说成新策略提升。已知退化和未完成实现继续留在隔离分支，不为提高提交频率混入候选。DSH 后续实验保留上下文并行推进；收到新回测后按实际版本继续修正，不等整批实验收尾。
