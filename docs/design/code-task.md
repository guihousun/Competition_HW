...
用户反馈「地图可视化效果比较差」。四个具体痛点：

1. **没法定位** —— `(14,14)` 在哪一格要数格子，而策略里**一切都是坐标**（任务点、建造环、炮位），
   复盘时数格子是每分钟都在付的成本。这是最大的那条。
2. 相邻单位糊在一起（`....gss` 读起来像一个三字符记号）。
3. **没有图例** —— `k` / `$` / `1`-`4` / `%` 得回源码查 `_RENDER_SIDED` / `_RENDER_NEUTRAL`。
4. **状态看不见** —— 金币 / 射程 / 机器人血量 / 任务点完全不在图上，也推不出来。

用户问答拍板两条：**紧凑 + 坐标标尺**（保持 1 字符/格，否决了"每格隔一个空格"——
可读性最好但字节 ×2 到 3.6 MB/局）；**摘要 + 每回合图例**。

### 产出

- **`game/map.py`** —— `render()` 改成返回**整个可打印块**：两行列标尺 + 左侧行号槽 + 网格。
  标尺放进 `render()` 而不是让 `app._log` 自己拼：列宽与缩进由 `size` 推导，
  **那是地图自己的知识**，`app` 不该知道"第几个字符才是 x=0"。
  行号槽宽度**由 `height` 推导**（`len(str(height-1))`），写死 2 在 `height > 100` 时会
  把网格整体推右一列 —— 而地图恒 41×32，**本地测不出来**。
  守卫用 `not self.cells` 而不是 `size`：高/宽为 0 时 `size` 看着合法但一格都没有。
  新增 `_NAMES`（类别 → 中文名）+ `_legend()` + 模块常量 `LEGEND`。
- **`game/world.py`** —— `Turn.summary()` 3 行 + `_listed()` 辅助 + 两个常量。
  放这里的理由与 `Map.render()` 在 `map.py` 完全相同：**领域对象自己格式化自己**，
  `app` 只管装配与红线（它原本一个 `gold`/`weapons`/`robots` 字段都不碰）。
- **`app.py`** —— `_log` 把**三块拼成一条**记录（摘要 / 图例 / 地图），`import LEGEND as map_legend`。
  头部顺序是"**摘要 → 图例 → 图**"：摘要第一行 = 回合号（`logging` 的时间戳前缀只加在
  **第一条物理行**上，按时间翻日志时正是这一行要定位），图例紧挨着图。
...
我第一版草稿写的正是"1/2=我方任务点"，是 Plan agent 复核时挑出来的。
我方可接的那两个点是 `Turn.task_points`（来自 `teamOur.playerTasks`，阵营已滤好），
与字符表里的 `1`-`4` 是两回事，所以摘要里的标签写「**可接任务点**」而不是「我方任务点」。
用例 `test_the_legend_names_the_task_points_by_faction` 专门钉这一条（含 `assertNotIn`）。

**摘要的三条格式化约定**，都为了"日志必须诚实"：

- **缺失不等于 0** —— `gold` / `round_no` 的 -1 是"字段缺失"，打成 `?`；
  打成 0 会让人以为真的一分钱没有。射程 -1 ⇒ `?`（够不着），≥ 100 ⇒ `∞`
  （地图对角线约 52；样例火箭是 `INT_MAX`，直打 `r2147483647` 每回合白占 10 字节）。
- **冷却只在 `> 0` 时出现**：-1 与 0 都是"没有冷却"，与 `model._weapons` 的 `> 0` 判据**同源**。
- **每个列表都有上界**（`SUMMARY_MAX_ITEMS = 8`，超出报 `…+n`）。`Turn.robots` 是
  **逐回合全量**推送的、`weapons` 也没有硬上界 —— 不截断的话摘要长度就是一个**数据相关的量**，
...
- ❌ **不做裁剪/缩放**（只画有内容的矩形框）—— 会裁掉远处正在逼近的机器人，
  而那是夜里最该看见的东西。
- ❌ **不引入任何跨回合状态**。图例每回合重复打（用户选定），不搞"只在第 1 回合打"的
  模块级标志位 —— 那正是第 11 步否决掉的那类状态。
...
**2. 反向验证（6 条，每条先改坏 → 确认挂了 → 复原，全部如实复现）**

| 故意改坏 | 结果 |
|---|---|
| `render()` 去掉 `reversed()` | `test_render_labels_every_row_with_its_y` 挂（行号变成 0→31 递增） |
| `_NAMES` 删掉 `weaponShop` | `test_the_legend_covers_every_category` 挂 |
| 图例写成"我方任务点1" | `test_the_legend_names_the_task_points_by_faction` 挂 |
| 射程 -1 直打成 `-1` | `test_the_unknown_range_is_not_printed_as_a_negative_number` 挂 |
| `_listed` 不截断 | `test_long_lists_are_capped` 挂 |
| `_log` 把 `summary()` 放到 `render()` 之后 | `test_every_round_logs_the_map_then_the_actions` 挂 |

**3. 逐字节回归** —— 这一步只动日志，响应一个字节都不该变。
`git archive HEAD`（第 11 步的提交）抽出一份旧树跑同一个样例，两边**都是 213 字节、内容相同**。

**4. 真服务端到端**（`netstat` 确认端口空闲 → `run.sh 18085` → `curl`）：
...
**行号与标尺都核对过**：加特林在 `(9,24)` ⇒ `24 │` 那一行第 10 个字符正是 `g`
...
| 摘要 | 294 | 3 |
| 图例 | 305 | 3 |
| 地图（2 行标尺 + 32 行） | 1661 | 34 |
| **合计** | **2262**（改前 1374） | **40**（改前 33） |

1300 回合 ≈ **2.9 MB**（改前 ≈ 1.7 MB）。⚠️ 与硬约束 5 直接相关：按 64 KB 管道缓冲算，
写满从**约 48 回合提前到约 29 回合**。**两种设计都是死**，所以这条不构成不做这一步的理由，
但**数字必须同步** —— 留着"32 行 / 2MB"会让下一个人按错的量级估风险。

> 估算两次都偏低：我按 46×34 + 换行算成 1596 字节，漏掉 `│` 是 **3 字节**（32 行 = +64）；
> 图例估 240、实测 305。**字节数一律以实测为准。**

### 已知不确定性（别假装确定）

1. **stdout 会不会把管道写满** —— 第 9 步就记了这条，这一步只是把阻塞点从约 48 回合
   提前到约 29 回合。**判题器读不读子进程的 stdout，本地验不了**（硬约束 5）。
   真出事只能把 `_log` 关掉或改写文件，一处可改。

2. **`│` 的显示宽度** —— 它在 UTF-8 终端占 1 列（本项目日志恒为 UTF-8），
   但在 cp936 终端是 2 列，那时行号槽与标尺会差一列。选它是因为用户拍板的预览里就是它；
   真要换，`render()` 里一处 + 用例里一处。

3. **宽度按字符数算而不是显示列数** —— 成立的前提是 `_char` 恒返回 1 字符且图里没有中文。
   这个不变量已经写进 `_char` 的 docstring（兜底是 `?` 不是 `""`），但**没有断言守着**。
...
② 要不要启用沙盒 `executeCmd`；③ `summonTreasure` / 宝藏那条线；
④ 若 `timeoutRounds` 真出现，可加"时间不够就不接"的取舍；
⑤ 复盘日志的体量要不要按实测（stdout 那条风险）收一收。
