...
用户手动改了日志的措辞与几处版面，本步把**改动的后果**补齐：让代码、日志、文档、用例
四边重新对齐。**策略与响应报文一个字节没动**（`roleCommandMap` / `prompt` / `executeCmd` 照旧）。

### 产出

- **日志标签统一成 `【…】：`**（用户手改）：`【判题器报错】：` / `【SOP 更新】：`。
- **任务行与沙盒行搬到 `planner.task_channel`**（用户决定"留在 planner"）：它们打的三样字段
  （`phase_task` / `llm_resp` / `cmd_result`）**只在那个函数的作用域里**，而这一搬带来三个后果，
  本步逐一补齐：
  - ⚠️ **必须写在早返回之前**：原来的改动插在 `if not turn.phase_task … return` **之后**，
    于是"任务刚结束、`phase_task` 已空但 `llm_resp` 还有值"那一回合——**唯一一次能看见
    判题器最后答了什么的机会**——永远打不出来。已上移。
  - ⚠️ **`assertLogs("coregeek.app")` 看不见它了**：这是本项目**第二条不在 `app` 名下的日志**，
    三条用例改成收 root（与第 18 步 SOP 那条同一个坑）。
  - **`prompt` 那一格留在 `app._log`**（`【本轮提问】：`）：`task_channel` 拿到的是返回值，
    它在函数体里还没有这个值；`LOG_PROMPT_MAX` 也就不用搬出 `app`（避免 `planner → app` 反向边）。
- **摘要去掉块间空行**（用户手改）：四块各占一行。⚠️ 手改时【可接任务点】被拼进了【机器】
  那一行（两个 f-string 隐式拼接）——已拆回独立一行。
...
```
干净回合（没回执、没任务）        物理行 44 ｜ 字节 2555 ｜ 约 26 回合写满 64KB
有回执（判题器报错 + 回执名单）    物理行 46 ｜ 字节 2637 ｜ 约 24 回合写满 64KB
任务在身、还没答过（提问那一轮）    物理行 69 ｜ 字节 6170 ｜ 约 10 回合写满 64KB
任务在身 + 沙盒结果顶格（回灌）     物理行 70 ｜ 字节 8572 ｜ 约 7 回合写满 64KB
最坏（+ 一回合 SOP 调用顶格）      物理行 71 ｜ 字节 8861 ｜ 约 7 回合写满 64KB
```

  **净减 2 行、字节反而 +138**：空行是 1 字节，而 `—` 是 3 字节/字。上限仍是 9300，
  **余量只剩 439** —— 这个上限已经不再是"抓结构性膨胀"的网（那是 1200 字节量级），
  只是个"别再多打一整块"的兜底。真嫌紧就调 `LOG_TEXT_MAX` / `LOG_PROMPT_MAX`。
  量法：`assertLogs(level="INFO")` 收 root，累加 `len(r.getMessage().encode("utf-8"))`，
  物理行 = 每条 `1 + 消息里的换行数`（首行带时间戳前缀）。

### 不做什么

- **不动 `render()` 的画法本身**：用户明说"我手动修正了，不用管"。本步只修它周围
  已经说了假话的 docstring（`_char` / `render` / `CLAUDE.md` 里"行号槽 + 两行列标尺"那三处）。
- **不动策略、不动报文、不动判据链的顺序**：`task_channel` 只多了两行 `LOGGER.info`，
  判据 ①~⑥ 与"② 必须压在 ③ 前"一个字没改。
- **不把 `LOG_PROMPT_MAX` 搬进 `utils.py`**：那样能让 planner 也打提问行，但会多一条边，
  而它现在只有 `app` 一个使用者。
- **不给 `render()` 补行号**：用户是有意去掉的（第 21 步"铺满"的教训同源 —— 版面靠边框与图例，
  不靠塞满）。若真需要按坐标查格子，`__repr__` 之前先想想是不是该直接看 `cells`。

### 验证

```
PYTHONUTF8=1 py -m unittest discover -s tests        # 214 条全绿
```

- **同一局面在新旧两版上各量一遍**（`git worktree add D:/tmp/headw -d HEAD` 取 7311226 的代码）：
  最坏那一格旧版 8723 / 71 行、新版 8861 / 71 行；干净回合旧版 2481 / 46 行、新版 2555 / 44 行。
  **旧版能量出 `CLAUDE.md` 里记的 8723 ⇒ 量法可信**，这不是"换个脚本就换个数"。
- 反向验证：把 `planner` 的任务行挪回早返回之后，`test_the_task_line_…` 必须挂 —— 实测挂在下
  `assertIn("【本轮任务】：无", task)`（`AssertionError: '【本轮任务】：无' not found in ''`），
  即"任务刚结束那一回合"整块日志消失。**改回后 214 条复绿。**
  ⚠️ 第一次写这个验证脚本时把新旧顺序写反了，脚本**什么也没改**、用例当然"过了" ——
  反向验证本身也要证伪（判据：脚本得打印出"真的换了"）。
- 真服务一局：`bash run.sh 18085` + `curl -s -X POST --data-binary @docs/request.txt …` ⇒
  响应与第 18 步逐字一致（三个顶层字段齐全、三条 `move` 与 `EXPECTED_MOVES` 相同），
  日志里摘要四行、图例一行 `【图例】：`、地图 34 行（`—` 边框）；跑完杀进程并确认端口释放。
...
同第 22 步的下一步：① 首场实盘看 `sell` 与铜的涨落；② `buy` / `use` 券线；③ 卖矿阈值调参。
