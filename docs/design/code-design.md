...
第 5 步产出了 `domain/{world,economy,assignment,planner}.py`，自检从 38 条涨到 **59 条**，
其中 `planner_one_round_produces_build_sell_and_buy_together` 是本步的验收用例
（两个白天回合的**并集**覆盖建墙 / 贩卖 / 采购三条指令 —— 为什么必须拆成两回合见 ⑤），
`economy_day_one_builds_weapons_before_anything_else` 是**开局行为**的验收用例。
落地时改掉了三处原设计，外加 ④⑤ 中由实盘反馈查出来的五个 bug：
...
于是分工定为（`planner._assign_jobs`，**按 `pending` 全局重算、不粘**，见 ⑤）：

| 角色 | 岗位 | 职责 |
|---|---|---|
| 工人 | `fortify`（**防御活没干完时，两个工人都是**） | **先建满 3 座武器** → 按批攒石 → 按 `gaps[i::n]` 补自己那份围墙缺口 |
| 工人 | `economy`（**只在防御活干完之后出现**） | 挖铁/铜（**不挖石头**）→ 满载 85% 后去小贩处清仓 |
| 开拓者 | `pioneer` | 去武器商店采购 → 回基地用券 → 白天预定位到夜间要操控的那座武器旁 |

经济手刻意**不挖石头**：石头只值 1 金、只用于建墙，让现金来源也变成建材是纯亏。
反之防御线的建造手**优先挖石头**（`stone_short=True`）：第 1 天金币已被 3 座武器吃满，
挖铜当天一分钱也换不成东西，而天黑前少砌一面墙，当夜就多挨一份伤害。
...
#### ⑤ 两个工人**一起上防御线**（用户实盘反馈的第二轮）

用户反馈原文："1、第一天只有一个工人在动，这很不合理 2、工人优先采集了铜矿，
第一天工人应该先建造武器，然后采集石头，在天黑到来前尽可能建造多的石墙，用于防御"。

两条都成立，而且**根因是同一个**：③ 里那套"工人 A 专砌墙、工人 B 专挖矿"的分工
默认了两个人可以**互相供货**，而游戏里**没有转移物品的指令**（`drop` 只丢不捡，
§4.4 动作表没有拾取）。于是：

- 工人 B 挖来的铜和石料，**一块也送不到**工人 A 手上；
- 第 1 天真正该抢的是"天黑前合拢正面那 6 面墙"，而防御线上只有一个人在干活。

改法是 `planner._defense_pending(turn, box, gaps)`：**武器还没建满 或 盒子还有缺口**
→ 两个工人都判 `fortify`；否则两个工人都判 `economy`。

```python
def _defense_pending(turn, box, gaps) -> bool:
    if next_weapon_build(turn, box) is not None:   # 开局 3 座武器也是防御活
        return True
    return bool(gaps)                              # gaps 已排除门
```

三个必须注意的点（每一条都踩过或差点踩到）：

1. **"防御活"必须包含武器**，不能只看围墙缺口。只看缺口的话，第 1 天盒子还没缺口，
   两个工人都会被判成多余的，武器照样没人建（与 ④ bug 1 是同一个坑的第二次出现）。
2. **岗位刻意不粘**（每次重算，与 ③ 里"跨回合粘住"相反）。夜里墙会被拆，
   缺口第二天会**重新出现** → 两个工人自动回到防御线补墙，补完再一起转经济。自平衡，
   不需要为"什么时候切回防御"再写一条状态。代价是分工在同一场内会来回切，
   但这正是想要的语义。
3. **认领制**（`claimed_sites` / `claimed_mines`）从"可选优化"升级为**必需**：
   两个建造手同时开工，没有认领就会同时冲向同一个武器空位/同一块矿，
   一个成功了，另一个的动作白扔。`_plan_day` 按角色 id 排序遍历，
   前一个人的 `Goal.target` 进入 `claimed_*`，后一个人通过 `avoid=` 避开。
   `next_weapon_build` 的**类型选择**也跟着改：`order[min(len(alive) + len(avoid), ...)]`
   —— 认领中的那几座还没落地，不这么改两人会都去建加特林。

行为面的验收（`selfcheck`）：

| 用例 | 断言 |
|---|---|
| `planner_two_workers_split_the_defense_line` | 第 1 天两个工人都 `fortify`、开拓者 `pioneer`，三人各有一条意图；两人目标格**不同**；相邻时各自 `Build` 出**不同的武器名**（`gatling`/`railgun`）；`gaps[0::2]` 与 `gaps[1::2]` 都落在正面 |
| `planner_economy_hand_appears_only_after_defense_is_done` | `closed_box` → 两个 `economy`；删掉正面一格墙 → 两个立刻回到 `fortify` |
| `planner_never_puts_the_pioneer_on_a_worker_job` | 四种白天局面 × 连续 3 回合，开拓者恒为 `pioneer`，工人恒为 `fortify`/`economy` |

⚠️ **`planner_one_round_produces_build_sell_and_buy_together` 因此必须拆成两回合**：
分工由 `pending` **全局**决定，`sell` 与 `build` 在同一回合里**互斥**
（回合 A 有防御活 → `build`+`buy`；回合 B `closed_box` → `sell`+`buy`），
断言改成**两回合的并集**覆盖 `{build, sell, buy}`。这不是给测试放水——
"一个白天回合里三条线同时产出"这个原目标在新分工下**在物理上就不存在**，
继续断言它只会得到一个永远为假的用例。

⚠️ 自检夹具 `_payload` 也跟着修了一处**会骗人的地方**：`closed_box=True` 铺的 19 面墙，
必须用**和 production 完全一样的方式**推盒子。`world.box_of` 会拿场上机器人的位置
覆盖先验正面（`front_from_positions`），正面一变**门就换了一条边**，要铺的 19 格跟着变。
夹具早先不传 threat（用的是先验正面），摆出来的"满墙"和 planner 看到的盒子差了整整一格门，
`_defense_pending` 因此**永远为真**，经济手岗位根本叫不出来 —— 用例失败的原因看起来是策略错，
实际上是夹具在撒谎。**教训：夹具只要重新实现一遍 production 的推导逻辑，就有第二次写错的机会；
能直接调用 production 的函数就直接调用。**
...
| 19 | **两个工人一起上防御线**（§15.3 ⑤）：防御活没干完时**两人都是建造手**，干完才一起转经济手；岗位按 `_defense_pending` 全局重算、**刻意不粘**（夜里墙被拆 → 第二天缺口重现 → 自动回防）；缺口按 `gaps[i::2]` 交错分配 | **§3** 角色分工表（工人两行合并为 ×2）、**§6.3** 追加 v0.4 修正块、**§8** 日程表 `1–8` 行 |
| 20 | **游戏里没有转移物品的指令**（`drop` 只丢不捡）→ 每个角色的背包就是自己的料仓，"专人供料"行不通 → 推翻 v0.3 的"工人 A 砌墙 / 工人 B 供料" | **§3** 角色分工表的引用块 |
