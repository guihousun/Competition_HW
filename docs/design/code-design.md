...
| 4 | `domain/intent.py` + `protocol/commands.py` + `transport/server.py` + `app.py` + `main.py` + `run.sh` | **此时已可起服务**：POST 样例返回 200、三字段齐、报文合法、耗时 <5s | ✅ 完成 |
...
### 15.3 第 5 步（经济线）落地时的三条修正

第 5 步产出了 `domain/{world,economy,assignment,planner}.py`，自检从 38 条涨到 **50 条**，
其中 `planner_one_round_produces_build_sell_and_buy_together` 是本步的验收用例
（一个白天回合同时产出建墙 / 贩卖 / 采购三条指令）。落地时改掉了三处原设计：
...
| 角色 | 岗位 | 职责 |
|---|---|---|
| 工人 A | `fortify` | 挖石头 → 按 `wall_order()` 补围墙盒（正面 → 两侧 → 背面，门除外） |
