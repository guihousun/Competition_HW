...
- 城墙升级依次考虑最前面的四堵墙、前沿两端的两堵拐角墙、其他墙；已经三级的墙退出候选，因此前四墙满级后会继续升级两个拐角。普通墙损耗不会抢走炮台升级预算；前六墙低于紧急阈值、基地濒危时保留应急例外。
...
“前六墙”由当前己方基地和迎敌方向计算：最前面一排的中间四格，加上该排两个端角；左右阵营镜像，纵向迎敌时整体旋转，水面或地图外位置不会补成新的建造格。保护范围不以墙是否已经造好为条件，关闭导航或允许拆墙开关也不解除前六墙保护。沿用的墙圈与迎敌方向仍属于 `DEVELOPMENT_RULES.md` D01 的本地几何假设，不能当作官方地图已对齐。
