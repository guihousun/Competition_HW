...
Issue 只需描述现象并附可公开日志；版本优先从启动 identity 读取，地图和版本可选。不需要先整理规则编号、失败原因或最小复现。
...
```powershell
python tools/trace_tool.py tasks $traceDir --output .\evidence\task-events.jsonl
```

该文件保留解题相关正文与请求/回执（通常不截断）、包身份、原日志捕获质量和断档信息，方便直接附到 Issue。它不执行模型、命令或策略，不自动上传，也不能从旧摘要恢复未记录的题目。已存在的目标文件不会被覆盖；内容仍需确认允许公开。
