...
if args.child:
    ...
    # Explicit experiment interventions; no source or simulator rules changed.
    if args.mode == 'no-preparation':
        brain._prepare_treasure = lambda *args, **kwargs: False
    elif args.mode == 'no-treasure-actions':
        brain._treasure_notes = lambda payload, turn: {'known': False, 'open': False, 'taken': False}
    elif args.mode == 'no-news-actions':
        brain._mine_available = lambda turn, material: True
    ...
    row = {'seed': args.seed, 'side': args.side, 'mode': args.mode, 'rounds': len(times),
           ...
           'stable_sources': initial == hashes(), 'files': initial, 'runtime_intervention': args.mode, 'driver_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
    ...
    jobs = [(seed, side, mode) for seed, side in ((90713, 'defender'), (90317, 'challenger'))
            for mode in ('no-preparation', 'no-treasure-actions', 'no-news-actions')]
    running, rows = [], []
    while jobs or running:
        ...
        for job in list(running):
            ...
            print(json.dumps({k: v for k, v in row.items() if k != 'files'}, ensure_ascii=False), flush=True)
    report = {'scope': 'Controlled runtime policy ablations of public-world consumers, with unchanged local rules and scripted cognition. Runtime interventions are explicit, not pristine source runs or intranet PASS.',
