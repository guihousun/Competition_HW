...
if args.child:
    ...
    jobs = [(seed, side, mode) for seed in (90317, 90601) for side in ('challenger', 'defender')
            for mode in ('baseline', 'tasks', 'all')]
    jobs += [(90601, side, 'all-faults') for side in ('challenger', 'defender')]
    running, rows = [], []
