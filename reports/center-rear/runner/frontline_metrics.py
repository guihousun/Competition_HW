...
def removal_errors(obs,commands):
    ...
    if base:
        x,y=xy(base['pos']);w,h=obs['mapInfo']['width'],obs['mapInfo']['height']
        cx,cy=(w-1)/2,(h-1)/2
        if x+1<cx:rear={(x-2,j) for j in range(y-3,y+3)}
        elif x>cx:rear={(x+3,j) for j in range(y-3,y+3)}
        elif cy!=y-.5:
            yy=y-3 if cy>y-.5 else y+2
            rear={(i,yy) for i in range(x-2,x+4)}
        else:
            xx=x-2 if cx>=x+.5 else x+3
            rear={(xx,j) for j in range(y-3,y+3)}
        for uid,c in commands.items():
            if c.get('action')=='build' and c.get('name')=='wall':
                for target in c.get('targetPos',[]):
                    if xy(target) in rear:errors.append({'role':str(uid),'kind':'rear_wall_build','target':list(xy(target))})
    return errors
