...
class SopStateTest(unittest.TestCase):
    ...
    `deposit` 落的是**暂存表** `_pre_sop`（判题器没报错的那一轮才转正进 `_sop`）⇒
    ...
    def test_a_new_flow_is_appended(self):
        """异名 ⇒ 追加一条（不同的经验各存各的）。"""
        self.agent.deposit("找文件", "第一步")
        self.agent.deposit("读题", "第二步")
    ...
    def test_each_instance_keeps_its_own_tool_table(self):
        """工具表也是实例的：`submitAnswer` 那一项是绑定方法，钉在各自的实例上。

        退化写法是共享一张模块级工具表、表里那个函数去写"某个全局答卷"（症状：两个
        Agent 的答卷互相覆盖）—— 这条走工具表路径、不直接调方法，把它挡住。
        """
        other = Agent()
        self.agent.tool_call("submitAnswer", [("answer", "甲交的")])
        other.tool_call("submitAnswer", [("answer", "乙交的")])
        self.assertEqual(self.agent.answer, "甲交的")
        self.assertEqual(other.answer, "乙交的")
    ...
    def test_a_deposit_stays_staged_until_a_clean_verdict(self):
        ...
        ③ 判决轮报 `code 2` ⇒ 这笔账结掉、暂存那条原地不动；④ 再交一次卷、干净 ⇒ 照旧转正。
        **转正走的是同一个 `store`**（② 里同名那条覆盖掉正式表的旧版）。
        """
        self.agent.deposit("甲", "第一版")
        self.agent.settle_deposit(True, True)
        ...
        self.agent.deposit("甲", "相似任务的第二版")
        self.agent.settle_deposit(False, False)  # 判决轮报了错 ⇒ 结账、不转正
    ...
    def test_an_empty_text_deletes_that_flow(self):
        ...
        self.agent.deposit("甲", "有内容")
        with self.assertLogs(sop.__name__, level="INFO") as caught:
            self.agent.deposit("甲", "")
        self.assertEqual(self.agent.pre_sop, {})
        self.assertEqual(len(caught.records), 1)
        with self.assertNoLogs(sop.__name__, level="INFO"):
            self.agent.deposit("不存在的", "")
        self.assertEqual(self.agent.pre_sop, {})
