...
class WallRepairTest(unittest.TestCase):
    ...
    def test_the_repairer_spends_a_pack_and_nothing_else(self):
        """修墙工这一夜只有包可用（墙券归开拓者买也归他用）⇒ 残墙前发出去的永远是 `WallFixer`。"""
        worker = Worker(2, self.POST, {WALL_FIXER: 2, "WallUpgradeVoucher1": 1})
        cmds = plan(self._night(worker, damaged={self.FRONT: (140, 1)}))
        self.assertEqual(cmds["2"], {"action": "use", "name": WALL_FIXER,
                                     "targetPos": [{"x": 13, "y": 23}]}, f"用包：{cmds}")
