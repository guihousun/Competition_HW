...
字节预算，以及任务线的端到端（handle 串起 web/protocol/game/agent，是唯一的集成面）。
...
class HandleTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # 这个文件里没有用例要看它：`executeCmd` 的断言全是"这一轮不该发命令"。
        cmd_explore.reset()
        cmd_explore.mute()
    ...
    def test_a_clean_round_logs_only_the_summary_and_the_actions(self):
        ...
        raw["worldNews"] = {"officialNews": ""}  # 没新闻 ⇒ 查价分支不触发
        with self.assertLogs("coregeek.app", level="INFO") as caught:
    ...
    def test_the_prompt_line_keeps_its_own_limit(self):
        ...
        # 首段的段头 `# 【` 开头 —— 段头前不许有空白，靠 `gen_system_prompt` 的 join。
        # 只钉"是个固定段头"，不绑具体哪一段：首段换人（第 133 步删了【背景】）不该动这条）
        self.assertTrue(
            asked.startswith('[{"role":"system","content":"# 【'), asked[:40]
        )
    ...
    def test_the_task_loop_through_handle(self):
        ...
        raw["lastCmdResult"] = ""
        raw["errors"] = []  # 样例自带一条假错误（CLAUDE.md 已声明别当真实信号读）
        for node in raw["teamOur"]["roles"]:
            if node["id"] == 10011:
